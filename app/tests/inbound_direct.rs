...
async fn direct_inbound_tcp_forwarding() {
    ...
    thread::spawn(move || {
        let inbound = sb_core::inbound::direct::DirectForward::new(listen, dst_host, dst_port, false);
        let _ = inbound.serve();
    });
    ...
    let mut stream = tokio::net::TcpStream::connect(("127.0.0.1", inbound_port)).await.unwrap();
