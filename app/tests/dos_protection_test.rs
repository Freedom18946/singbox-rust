...
async fn test_connection_flood_protection() {
    ...
    println!("✅ Connection flood test: {} successful, {} blocked (rate limited)", 
             successful, blocked);
    
    // Verify rate limiting is working (some connections should be blocked)
    assert!(blocked > 0, "Expected some connections to be rate-limited, got 0");
    assert!(successful > 0, "Expected some connections to succeed");
    
    // Successful connections should be roughly within rate limit
    assert!(successful <= 30, "Too many connections succeeded ({}), rate limiter may not be working", successful);
}
...
async fn test_resource_exhaustion_memory() {
    ...
    println!("✅ Resource exhaustion test: {}/200 connections established", count);
...
async fn test_burst_traffic_handling() {
    ...
    assert!(count <= 70, "Too many connections succeeded: {}, rate limiter may be broken", count);
}
...
async fn test_recovery_after_flood() {
    ...
    assert!(result.is_ok(), "✅ System should recover after attack and allow normal traffic");
