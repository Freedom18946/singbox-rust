...
async fn test_trojan_shadowsocks_2hop_performance() {
    ...
    for size in payload_sizes {
        ...
        println!("  Latency: {:.2} ms per request", elapsed.as_secs_f64() * 1000.0 / iterations as f64);
    }
...
async fn test_shadowsocks_trojan_2hop_performance() {
    ...
    let elapsed = start.elapsed();
    let throughput = (payload.len() as f64 * iterations as f64) / elapsed.as_secs_f64() / (1024.0 * 1024.0);
    
    println!("Completed {} requests", iterations);
    println!("Throughput: {:.2} MB/s", throughput);
    println!("Avg latency: {:.2} ms", elapsed.as_secs_f64() * 1000.0 / iterations as f64);
...
async fn test_multihop_overhead_measurement() {
    ...
    println!("2-hop: {:.2} ms ({:.1}% of single)", two_hop.as_secs_f64() * 1000.0, 
             (single_hop.as_secs_f64() / two_hop.as_secs_f64()) * 100.0);
    println!("3-hop: {:.2} ms ({:.1}% of single)", three_hop.as_secs_f64() * 1000.0,
...
async fn test_1000_concurrent_connections_sustained() {
    ...
    let test_duration = Duration::from_secs(10); // Shortened for CI/testing
    ...
    println!("Success rate: {:.1}%", (successful as f64 / num_connections as f64) * 100.0);
    println!("Total requests: {}", total_requests);
    println!("Duration: {:.2}s", elapsed.as_secs_f64());
    println!("Requests/sec: {:.2}", total_requests as f64 / elapsed.as_secs_f64());
    
    assert_eq!(successful, num_connections, "All connections should complete");
...
async fn test_10000_requests_per_second() {
    ...
    println!("Error rate: {:.3}%", (error_count as f64 / request_count as f64) * 100.0);
    
    assert!(actual_qps >= target_qps as f64 * 0.95, "Should achieve ≥95% of target QPS");
...
async fn test_spike_0_to_5000_connections() {
    ...
    for batch in 0..batches {
        ...
        println!("Batch {}/{}: {} connections spawned", batch + 1, batches, (batch + 1) * connections_per_batch);
    ...
    assert!(successful >= target_connections * 95 / 100, "≥95% success rate required");
...
async fn test_connection_establishment_latency() {
    ...
    println!("Trojan TLS handshake: {:.2} ms (target: ≤10ms)", trojan_latency);
    
    assert!(ss_latency < 1.0, "Shadowsocks handshake should be <1ms");
    assert!(trojan_latency < 10.0, "Trojan TLS handshake should be <10ms");
