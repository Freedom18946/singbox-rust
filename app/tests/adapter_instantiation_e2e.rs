...
fn test_feature_gate_control() {
    ...
    #[cfg(feature = "adapter-http")]
    {
        ...
        let ir = to_ir_v1(&config);
        let result = Bridge::new_from_config(&ir);
        assert!(result.is_ok(), "HTTP adapter should be available when feature is enabled");
    }
    ...
    #[cfg(feature = "out_tuic")]
    {
        ...
        let ir = to_ir_v1(&config);
        let result = Bridge::new_from_config(&ir);
        assert!(result.is_ok(), "TUIC adapter should be available when feature is enabled");
    }
    ...
    #[cfg(feature = "out_hysteria2")]
    {
        ...
        let ir = to_ir_v1(&config);
        let result = Bridge::new_from_config(&ir);
        assert!(result.is_ok(), "Hysteria2 adapter should be available when feature is enabled");
    }
}
...
fn test_adapter_registry_robustness() -> Result<()> {
    ...
    let result = Bridge::new_from_config(&ir);
    assert!(result.is_ok(), "Bridge should handle minimal config gracefully");
...
fn test_stub_adapter_warnings() {
    ...
    let stub_inbounds = vec![
        "naive", "shadowtls", "hysteria", "hysteria2", "tuic", "anytls"
    ];
    ...
    // Test stub outbounds
    let stub_outbounds = vec![
        "tor", "anytls", "wireguard"
    ];
...
fn test_adapter_registration_idempotent() {
    ...
    let ir = to_ir_v1(&config);
    let result = Bridge::new_from_config(&ir);
    assert!(result.is_ok(), "Adapters should work after multiple registrations");
}
