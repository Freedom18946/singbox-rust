...
pub fn emit<T: Serialize>(fmt: Format, human: impl FnOnce() -> String, json: &T) {
    match fmt {
        ...
        Format::Json => println!("{}", serde_json::to_string_pretty(json).unwrap_or_else(|_| "{}".into())),
        Format::Sarif => println!("{}", serde_json::to_string_pretty(json).unwrap_or_else(|_| "{}".into())), // route 先复用JSON，check单独SARIF
    }
}
