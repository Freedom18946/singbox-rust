...
pub(crate) fn compute_hist(lat_ms: &[u64], buckets: &[f64]) -> Hist {
    ...
    for &c in &counts {
        acc += c;
        cdf.push(if total == 0.0 { 0.0 } else { (acc as f64 / total).min(1.0) });
    }
...
async fn bench_io(url: String, requests: u32, concurrency: usize, json: bool,
    ...
    out.hist = if let Some(spec) = hist_buckets {
        ...
        for &lat_ms in &v {
            let mut idx = buckets.len().saturating_sub(1);
            for (i, &b) in buckets.iter().enumerate() {
                if (lat_ms as f64) <= b { idx = i; break; }
            }
            if idx < counts.len() { counts[idx] += 1; }
        }
...
#[cfg(not(feature = "reqwest"))]
async fn bench_io(_url: String, _requests: u32, _concurrency: usize, _json: bool,
...
mod tests {
    ...
    fn test_json_roundtrip_with_hist() {
        ...
        let s = IoStats {
            total: 9, ok_2xx: 8, other: 1,
            elapsed_ms: 1234, bytes: 4096,
            p50_ms: 10, p90_ms: 50, p99_ms: 90, max_ms: 100, min_ms: 1,
            rps: 100.0, thrpt_mib_s: 1.23,
            hist: Some(h.clone()),
        };
