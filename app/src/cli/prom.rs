...
pub enum PromCmd {
    ...
    ///   若分组 label 缺失则归为组名 "_"
    Hist {
        #[arg(long="metric")] metrics: Vec<String>,
        #[arg(long)] url: String,
        #[arg(long="label")] labels: Vec<String>,
        /// 对直方图进行分组聚合的标签键
        #[arg(long="group-by")] group_by: Option<String>,
        /// 输出格式（默认 text）
        #[arg(long, value_enum, default_value_t=HistFormat::Text)] format: HistFormat,
    },
}

#[derive(Copy, Clone, Eq, PartialEq, Debug, ValueEnum)]
pub enum HistFormat { Text, Json }
impl fmt::Display for HistFormat {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self { HistFormat::Text => f.write_str("text"), HistFormat::Json => f.write_str("json") }
    }
}
...
pub async fn main(a: PromArgs) -> Result<()> {
    match a.cmd {
        ...
        PromCmd::Hist { url, metrics, labels, group_by, format } => {
...
async fn scrape(url: String, filter: Option<String>, select: Option<String>, labels: Vec<String>, jsonl: bool, _json: bool) -> Result<()> {
    ...
    if !jsonl {
        if let Some(sel) = sel {
            ...
            for s in &out { println!("{}\t{}\t{}", s.name, serde_json::to_string(&s.labels)?, s.value); }
        }
    }
    Ok(())
}

async fn hist(metrics: Vec<String>, url: String, labels: Vec<String>, group_by: Option<String>, format: HistFormat) -> Result<()> {
    let txt = http_get_text(&url).await?;
    let label_sel = parse_label_kvs(&labels);
    #[derive(Default)]
    struct Agg { buckets: Vec<(f64,f64)>, sum: f64, count: f64 }
    #[derive(Serialize)]
    struct GroupOut<'a> { metric: &'a str, group: String, p50: f64, p90: f64, p99: f64, sum: f64, count: f64 }
    ...
    let rx_for = |m: &str| -> Result<(Regex,Regex,Regex)> {
        Ok((
            Regex::new(&format!(r#"^{}_bucket\{{([^}}]*)\}}\s+([-\+]?\d+(\.\d+)?)$"#, regex::escape(m)))?,
            Regex::new(&format!(r#"^{}_sum\{{([^}}]*)\}}\s+([-\+]?\d+(\.\d+)?)$"#, regex::escape(m)))?,
            Regex::new(&format!(r#"^{}_count\{{([^}}]*)\}}\s+([-\+]?\d+(\.\d+)?)$"#, regex::escape(m)))?
        ))
    };
    let mut rx: HashMap<String,(Regex,Regex,Regex)> = HashMap::new();
    for m in &metrics { rx.insert(m.clone(), rx_for(m)?); }
    ...
    for line in txt.lines() {
        ...
        for m in &metrics {
            ...
            if let Some(cap) = re_b.captures(line) {
                ...
                if !label_sel.is_empty() && !labels_match(&lbl, &label_sel) { continue; }
                let group_key = group_by.as_ref()
                    .and_then(|k| lbl.get(k).cloned())
                    .unwrap_or_else(|| "_".into());
                let mut le = f64::INFINITY;
                if let Some(v) = lbl.get("le") { le = v.parse::<f64>().unwrap_or(f64::INFINITY); }
                let v = cap[2].parse::<f64>().unwrap_or(0.0);
                let entry = store.entry(m.clone()).or_default().entry(group_key).or_default();
    ...
    for (metric, groups) in store.iter_mut() {
        for (gk, agg) in groups {
            ...
            for (le,v) in &buckets { _noncum.push((*le, (*v - prev).max(0.0))); prev = *v; }
            ...
            outs.push(GroupOut {
                metric, group: gk.clone(), p50, p90, p99, sum: agg.sum, count: agg.count
            });
        }
    }
    match format {
        HistFormat::Text => {
            for o in outs {
                if group_by.is_some() {
                    println!("metric={} group={} p50={} p90={} p99={} sum={} count={}",
                        o.metric, o.group, o.p50, o.p90, o.p99, o.sum, o.count);
                } else {
                    println!("metric={} p50={} p90={} p99={} sum={} count={}",
...
mod tests {
    ...
    fn test_label_selector_match() {
        let all = BTreeMap::from([("job".into(),"api".into()), ("instance".into(),"x:123".into())]);
