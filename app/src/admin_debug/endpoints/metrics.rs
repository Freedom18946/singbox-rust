...
pub async fn handle(sock: &mut (impl AsyncWriteExt + Unpin)) -> std::io::Result<()> {
    ...
    buf.push_str(&line("sb_subs_connect_timeout_total", h.subs_connect_timeout));
    ...
    buf.push_str(&line("sb_subs_block_private_total", h.subs_block_private_ip));
    ...
    buf.push_str(&format!("sb_subs_cache_evict_total{{tier=\"mem\"}} {}\n", h.subs_cache_evict_mem));
    buf.push_str(&format!("sb_subs_cache_evict_total{{tier=\"disk\"}} {}\n", h.subs_cache_evict_disk));
    buf.push_str("# HELP sb_subs_cache_bytes cache byte usage by tier\n# TYPE sb_subs_cache_bytes gauge\n");
    buf.push_str(&format!("sb_subs_cache_bytes{{tier=\"mem\"}} {}\n", h.cache_bytes_mem));
    buf.push_str(&format!("sb_subs_cache_bytes{{tier=\"disk\"}} {}\n", h.cache_bytes_disk));
    buf.push_str("# HELP sb_subs_head_total HEAD request total\n# TYPE sb_subs_head_total counter\n");
    ...
    for (le, c) in h.dns_latency_buckets.iter() {
        let bucket = if *le >= 999999.0 { "+Inf".to_string() } else { le.to_string() };
        buf.push_str(&format!("sb_subs_dns_resolve_seconds_bucket{{le=\"{}\"}} {}\n", bucket, c));
    }
    buf.push_str(&format!("sb_subs_dns_resolve_seconds_count {}\n", h.dns_latency_count));
    buf.push_str(&format!("sb_subs_dns_resolve_seconds_sum {}\n", h.dns_latency_sum_ms as f64 / 1000.0));
    ...
    for (k, v) in h.error_kinds.iter() {
        buf.push_str(&format!("sb_subs_error_kind_total{{kind=\"{}\"}} {}\n", k.as_str(), v));
    }
    ...
    for ((kind, host_hash), count) in h.error_kinds_by_hash.iter() {
        buf.push_str(&format!("sb_subs_error_kind_by_hash_total{{kind=\"{}\",host_hash=\"{}\"}} {}\n", kind.as_str(), host_hash, count));
    }

    // Request latency histogram with finer buckets
    buf.push_str("# HELP sb_subs_fetch_seconds request latency\n# TYPE sb_subs_fetch_seconds histogram\n");
    for (le, c) in h.latency_buckets.iter() {
        let bucket = if *le >= 999999.0 { "+Inf".to_string() } else { format!("{:.3}", le) };
        buf.push_str(&format!("sb_subs_fetch_seconds_bucket{{le=\"{}\"}} {}\n", bucket, c));
    }
