//! Tools CLI: connect (minimal parity)
//!
//! Usage examples:
//! - cargo run --bin tools -- connect example.com:80 -c config.json
//! - cargo run --bin tools -- connect 1.1.1.1:443 -c config.json --outbound proxy
...
async fn main() -> Result<()> {
    let args = Args::parse();
    match args.command {
        Cmd::Connect { addr, network, config, outbound } => connect(addr, network, config, outbound).await,
        Cmd::Fetch { url, output } => fetch(url, output).await,
        Cmd::Synctime { server, timeout } => synctime(server, timeout).await,
        Cmd::FetchHttp3 { url, output } => fetch_http3(url, output).await,
    }
}

async fn connect(addr: String, network: Net, config: PathBuf, outbound: Option<String>) -> Result<()> {
    match network {
...
async fn connect_tcp(addr: String, config_path: PathBuf, outbound: Option<String>) -> Result<()> {
    // Load config JSON
    let raw = std::fs::read(&config_path).with_context(|| format!("read {}", config_path.display()))?;
    ...
    let connector = if let Some(name) = outbound {
        bridge.get_member(&name).or_else(|| bridge.find_direct_fallback()).context("outbound not found and no direct fallback")?
    } else {
        bridge.find_direct_fallback().or_else(|| bridge.get_member("direct")).context("no direct outbound found")?
    };

    // Dial
    let stream = connector.connect(&host, port).await.context("connect failed")?;
    ...
    let a = tokio::spawn(async move {
        ...
        loop {
            match tokio::io::AsyncReadExt::read(&mut stdin, &mut buf).await {
                Ok(0) => break,
                Ok(n) => {
                    if s1.send(&buf[..n]).await.is_err() { break; }
                }
                Err(_) => break,
            }
        }
    ...
    let b = tokio::spawn(async move {
        ...
        loop {
            ...
            match timeout(Duration::from_millis(500), s2.recv(&mut buf)).await {
                Ok(Ok(n)) => {
                    if n == 0 { break; }
                    if tokio::io::AsyncWriteExt::write_all(&mut stdout, &buf[..n]).await.is_err() { break; }
                }
                Ok(Err(_)) => break,
                Err(_) => {
...
async fn fetch(url: String, output: Option<PathBuf>) -> Result<()> {
    ...
    if let Some(p) = output {
        ...
        eprintln!("{} {} -> {} bytes -> {}", status.as_u16(), url, bytes.len(), p.display());
    } else {
...
async fn fetch_http3(url: String, output: Option<PathBuf>) -> Result<()> {
    ...
    let mut builder = reqwest::Client::builder()
        .user_agent(format!("singbox-rust-tools/{} (+http3)", env!("CARGO_PKG_VERSION")));
    // If compiled with reqwest_unstable cfg, prefer H3 directly.
