...
async fn main() -> Result<()> {
    ...
    match args.command {
        ...
        Commands::Decompile { file, output } => decompile_ruleset(file, output).await,
        Commands::Match { file, domain, ip, port, network } => match_ruleset(file, domain, ip, port, network).await,
        Commands::Compile { input, output, version } => compile_ruleset(input, output, version).await,
        Commands::Convert { input, output, version } => convert_ruleset(input, output, version).await,
        Commands::Merge { inputs, output, version } => merge_rulesets(inputs, output, version).await,
        Commands::Upgrade { input, output, version } => upgrade_ruleset(input, output, version).await,
    }
}
        ...
        Rule::Default(r) => {
            ...
            for d in &r.domain {
                match d {
                    ...
                    DomainRule::Suffix(s) => if !domain_suffix.contains(s) { domain_suffix.push(s.clone()); },
                    DomainRule::Keyword(s) => if !domain_keyword.contains(s) { domain_keyword.push(s.clone()); },
                    DomainRule::Regex(s) => if !domain_regex.contains(s) { domain_regex.push(s.clone()); },
                }
            }
            ...
            if r.invert { obj["invert"] = serde_json::json!(true); }
            if !domain_exact.is_empty() { let mut v = domain_exact; v.sort(); obj["domain"] = serde_json::json!(v); }
            if !domain_suffix.is_empty() { let mut v = domain_suffix; v.sort(); obj["domain_suffix"] = serde_json::json!(v); }
            if !domain_keyword.is_empty() { let mut v = domain_keyword; v.sort(); obj["domain_keyword"] = serde_json::json!(v); }
            if !domain_regex.is_empty() { let mut v = domain_regex; v.sort(); obj["domain_regex"] = serde_json::json!(v); }
            if !ip_cidr.is_empty() { obj["ip_cidr"] = serde_json::json!(ip_cidr); }
            if !r.port.is_empty() { obj["port"] = serde_json::json!(r.port); }
            if !r.port_range.is_empty() { obj["port_range"] = serde_json::json!(r.port_range); }
            if !r.network.is_empty() { obj["network"] = serde_json::json!(r.network); }
            if !r.process_name.is_empty() { obj["process_name"] = serde_json::json!(r.process_name); }
            if !r.process_path.is_empty() { obj["process_path"] = serde_json::json!(r.process_path); }
            obj
        }
        Rule::Logical(r) => {
            let rules: Vec<_> = r.rules.iter().map(rule_to_json).collect();
            let mode = match r.mode { LogicalMode::And => "and", LogicalMode::Or => "or" };
            let mut obj = serde_json::json!({
                "type": "logical",
                "mode": mode,
                "rules": rules,
...
async fn match_ruleset(
    ...
    let fmt = source::infer_format_from_path(file.to_str().unwrap_or(""))
        .context("Failed to infer input format")?;
    let ruleset = binary::load_from_file(&file, fmt).await.context("load rule-set")?;
...
async fn compile_ruleset(input: PathBuf, output: PathBuf, version: Option<u8>) -> Result<()> {
    ...
    let ver = version.unwrap_or(rs.version.max(sb_core::router::ruleset::RULESET_VERSION_CURRENT));
...
async fn merge_rulesets(inputs: Vec<PathBuf>, output: PathBuf, version: Option<u8>) -> Result<()> {
    use sb_core::router::ruleset::{binary, source, RuleSetFormat};
    if inputs.is_empty() { anyhow::bail!("no inputs provided"); }
    let out_fmt = source::infer_format_from_path(output.to_str().unwrap_or(""))
        .ok_or_else(|| anyhow::anyhow!("cannot infer output format"))?;
    ...
    if let Some(v) = version { out_ver = v; }
    match out_fmt {
        RuleSetFormat::Binary => {
        ...
        RuleSetFormat::Source => {
            let mut arr = Vec::new();
            for r in &all_rules { arr.push(rule_to_json(r)); }
...
async fn upgrade_ruleset(input: PathBuf, output: PathBuf, version: Option<u8>) -> Result<()> {
    ...
    match out_fmt {
        ...
        RuleSetFormat::Source => {
            let mut arr = Vec::new();
            for r in &rs.rules { arr.push(rule_to_json(r)); }
