...
use sb_core::context::{
    ManagedApiActiveGeneration, ManagedApiActivePhase, ManagedApiExit, ManagedApiExitRecord,
    ManagedApiRuntimeSnapshot, ManagedApiServer,
};
use sb_core::v2ray_stats::StatsManager;
#[cfg(any(feature = "v2ray-api", test))]
...
#[cfg(feature = "v2ray-api")]
use crate::services::v2ray_stats_proto::{
...
struct V2RayLifecycle {
    ...
    last_exit: Option<ManagedApiExitRecord>,
}
...
impl V2RayLifecycle {
    ...
    fn snapshot(&self) -> ManagedApiRuntimeSnapshot {
        ManagedApiRuntimeSnapshot {
            current: self.current.as_ref().map(|g| ManagedApiActiveGeneration {
...
pub struct V2RayApiServer {
    ...
    runtime_tx: watch::Sender<ManagedApiRuntimeSnapshot>,
}
...
impl V2RayApiServer {
    ...
    pub fn new(cfg: V2RayApiIR) -> Self {
        ...
        let (runtime_tx, _) = watch::channel(ManagedApiRuntimeSnapshot::default());
        ...
        match outcome {
            Ok(Ok(())) => {
                if shutdown_requested {
                    ManagedApiExit::CleanShutdown
                } else {
                    ManagedApiExit::UnexpectedCompletion
                }
            }
            Ok(Err(e)) => ManagedApiExit::ServeError(e.to_string()),
            Err(join_err) => {
                if join_err.is_panic() {
                    ...
                    ManagedApiExit::Panicked(msg)
                } else if join_err.is_cancelled() {
                    ManagedApiExit::Cancelled
                } else {
    ...
    fn commit_terminal(
        ...
        if replace {
            lc.last_exit = Some(ManagedApiExitRecord { generation, exit });
        }
...
impl ManagedApiServer for V2RayApiServer {
    ...
    fn start(&self) -> anyhow::Result<()> {
        ...
        // start publishes Running(g); there is no monitor, so close() commits the terminal.
        {
            ...
            if let Some(cur) = &lc.current {
                if cur.phase == ManagedApiActivePhase::Running {
                    return Ok(());
                }
            }
        ...
        tracing::info!(
            ...
            "V2Ray API server start requested (stub - enable 'v2ray-api' feature)"
        );
        Ok(())
    }
    ...
    fn close(&self) -> anyhow::Result<()> {
        ...
        // commits the terminal (generation-checked, mirroring commit_terminal).
        {
            ...
            if replace {
                lc.last_exit = Some(ManagedApiExitRecord {
                    generation,
    ...
    fn close(&self) -> anyhow::Result<()> {
        ...
        let shutdown_tx;
        {
            ...
            match lc.current.as_mut() {
                None => return Ok(()),
                Some(cur) => {
                    if cur.phase == ManagedApiActivePhase::ShutdownRequested {
                        // Already requested for this generation; do not resend.
                        return Ok(());
                    }
                    cur.phase = ManagedApiActivePhase::ShutdownRequested;
    ...
    fn subscribe_runtime_state(&self) -> Option<watch::Receiver<ManagedApiRuntimeSnapshot>> {
...
#[cfg(feature = "v2ray-api")]
#[derive(Debug)]
/// Tonic handler backed by shared V2Ray-compatible statistics.
...
mod tests {
    ...
    fn snap(server: &V2RayApiServer) -> ManagedApiRuntimeSnapshot {
        server
    ...
    fn last_exit_only_advances_by_generation() {
        ...
        V2RayApiServer::commit_terminal(
            ...
            &server.runtime_tx,
            1,
            ManagedApiExit::ServeError("late".into()),
        );

        let s = snap(&server);
        assert_eq!(
            s.last_exit,
            Some(ManagedApiExitRecord {
                generation: 2,
                exit: ManagedApiExit::CleanShutdown,
            }),
    ...
    fn publish_snapshot_locked_sends_current_state() {
        ...
        assert_eq!(
            ...
            Some((2, ManagedApiActivePhase::Running)),
    ...
    async fn normal_close_publishes_shutdown_then_clean_exit() {
        ...
        assert_eq!(
            s.current.map(|g| (g.generation, g.phase)),
            Some((1, ManagedApiActivePhase::ShutdownRequested))
        );
    ...
    async fn task_exit_without_close_is_unexpected_completion() {
        ...
        assert_eq!(
            ...
            Some((1, ManagedApiExit::UnexpectedCompletion)),
    ...
    async fn start_publishes_running_synchronously() {
        ...
        assert_eq!(
            ...
            Some((1, ManagedApiActivePhase::Running))
        );
        server.close().unwrap();
        assert_eq!(
            snap(&server).current.map(|g| g.phase),
            Some(ManagedApiActivePhase::ShutdownRequested)
        );
