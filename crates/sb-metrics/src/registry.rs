...
impl LabeledGauges {
    pub fn set(&self, labels: &[(&str, &str)], v: f64) {
        for (k, _) in labels {
            assert!(is_label_allowed(k), "label '{}' not allowed", k);
        }
...
pub fn export_prometheus() -> String {
    ...
    for (labels, v) in r.udp_fail_total.snapshot() {
        let lbl = labels
            ...
            .collect::<Vec<_>>()
            .join(",");
        out.push_str(&format!("{}{{{}}} {}\n", UDP_UPSTREAM_FAIL_TOTAL, lbl, v));
    }
    out.push_str(&format!("# TYPE {} histogram\n", UDP_TTL_SECONDS));
    for (bucket, cnt) in r.udp_ttl_seconds.snapshot() {
        out.push_str(&format!(
            "{}_bucket{{le=\"{}\"}} {}\n",
            UDP_TTL_SECONDS, bucket, cnt
        ));
    }
    out.push_str(&format!(
        "# TYPE {} counter\n{} {}\n",
