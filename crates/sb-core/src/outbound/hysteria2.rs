...
pub struct Hysteria2Outbound {
    ...
    pub connection_pool: Arc<Mutex<Option<Connection>>>,
...
impl Hysteria2Outbound {
    ...
    pub fn generate_auth_hash(&self) -> [u8; 32] {
    ...
    pub fn apply_obfuscation(&self, data: &mut [u8]) {
    ...
    pub async fn get_connection(&self) -> io::Result<Connection> {
...
pub mod inbound {
    ...
    impl Hysteria2Inbound {
        ...
        /// Accept a UDP datagram for UDP relay
        pub async fn accept_udp(&self) -> io::Result<(Vec<u8>, SocketAddr, [u8; 8])> {
            let endpoint = {
                let ep = self.endpoint.lock().await;
                ep.as_ref()
                    .ok_or_else(|| {
            ...
            session_id.copy_from_slice(&data[0..8]);

            // Return payload and peer address
            let payload = data[8..].to_vec();
            Ok((payload, peer, session_id))
        }
