...
fn try_scaffold_inbound(p: &InboundParam, _engine: crate::routing::engine::Engine<'_>, br: &Bridge) -> Option<Arc<dyn InboundService>> {
    if p.kind == "socks" {
...
fn try_scaffold_inbound(_p: &InboundParam, _br: &Bridge) -> Option<Arc<dyn InboundService>> { None }
...
pub fn build_bridge<'a>(cfg: &'a ConfigIR, engine: crate::routing::engine::Engine<'a>) -> Bridge {
    ...
    for ib in &cfg.inbounds {
        ...
        let inst: Option<Arc<dyn InboundService>> = match forced {
            ...
            None => try_adapter_inbound(&p).or_else(|| try_scaffold_inbound(&p, engine.clone(), &br)),
        };
