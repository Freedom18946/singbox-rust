...
impl Doh3Transport {
    ...
    async fn query_once(&self, packet: &[u8]) -> Result<Vec<u8>> {
        ...
        let (mut driver, mut send_request) = h3::client::new(h3_quinn::Connection::new(conn))
            .await
            .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("H3 handshake failed: {}", e)))?;
        ...
        let mut stream = send_request
            .send_request(req)
            .await
            .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("H3 send_request failed: {}", e)))?;

        // Send DNS query data
        stream
            .send_data(Bytes::copy_from_slice(packet))
            .await
            .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("H3 send_data failed: {}", e)))?;

        stream
            .finish()
            .await
            .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("H3 finish failed: {}", e)))?;

        // Receive HTTP/3 response
        let resp = stream
            .recv_response()
            .await
            .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("H3 recv_response failed: {}", e)))?;
