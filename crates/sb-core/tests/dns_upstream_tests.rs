...
use anyhow::Result;
use sb_core::dns::upstream::DnsUpstream;
...
async fn test_dot_upstream_construction() {
    ...
    let upstream = sb_core::dns::upstream::DotUpstream::new(server, "cloudflare-dns.com".to_string());
...
async fn test_doh_upstream_construction() -> Result<()> {
    ...
    let upstream = sb_core::dns::upstream::DohUpstream::new(
...
async fn test_doq_upstream_construction() -> Result<()> {
    ...
    let upstream = sb_core::dns::upstream::DoqUpstream::new(
        server,
...
async fn test_udp_upstream_real_query() -> Result<()> {
    ...
    assert!(!answer.addrs.is_empty(), "DNS query should return at least one address");
...
async fn test_doh3_upstream_real_query() -> Result<()> {
    ...
    let has_cloudflare = answer.addrs.iter().any(|addr| {
        matches!(addr, IpAddr::V4(v4) if v4.octets() == [1, 1, 1, 1])
    });
...
async fn test_doh_vs_doh3_consistency() -> Result<()> {
    ...
    let doh3_answer = doh3_upstream.query("one.one.one.one", RecordType::A).await?;

    // Both should return 1.1.1.1
    assert!(!doh_answer.addrs.is_empty());
    assert!(!doh3_answer.addrs.is_empty());

    let doh_has_cloudflare = doh_answer.addrs.iter().any(|addr| {
        matches!(addr, IpAddr::V4(v4) if v4.octets() == [1, 1, 1, 1])
    });
    let doh3_has_cloudflare = doh3_answer.addrs.iter().any(|addr| {
        matches!(addr, IpAddr::V4(v4) if v4.octets() == [1, 1, 1, 1])
    });

    assert!(doh_has_cloudflare && doh3_has_cloudflare,
