...
impl<D: Dialer + Send + Sync + 'static> crate::connector::OutboundConnector for TrojanConnector<D> {
    async fn connect(&self, target: &Target) -> Result<Box<dyn crate::connector::IoStream>, ProtoError> {
        let mut s = self.dialer.connect(&target.host, target.port).await
            .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e.to_string()))?;
        let hello = TrojanHello{ password: self.password.clone(), host: target.host.clone(), port: target.port };
