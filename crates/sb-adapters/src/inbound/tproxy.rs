...
#[derive(Clone, Debug)]
pub struct TproxyConfig { pub listen: SocketAddr }
...
pub async fn serve(cfg: TproxyConfig, mut stop_rx: mpsc::Receiver<()>) -> Result<()> {
    ...
    loop {
        select! {
            ...
            r = listener.accept() => {
                let (cli, peer) = match r { Ok(v) => v, Err(e) => { warn!(error=%e, "tproxy: accept error"); continue; } };
                tokio::spawn(async move {
                    if let Err(e) = handle_conn(cli, peer).await { warn!(%peer, error=%e, "tproxy: session error"); }
                });
            }
        }
    }
...
async fn handle_conn(mut cli: TcpStream, peer: SocketAddr) -> Result<()> {
    ...
    if let Some(eng) = rules_global::global() {
        ...
        if let Some(name) = sb_core::dns::fakeip::to_domain(&orig.ip()) { domain_opt = Some(name); }
        let ctx = RouteCtx { domain: domain_opt, ip: if domain_opt.is_some() { None } else { Some(orig.ip()) }, transport_udp: false, port: Some(port), process_name: None, process_path: None };
        let d = eng.decide(&ctx);
        if matches!(d, RDecision::Reject) { return Err(anyhow!("tproxy: rejected by rules")); }
        decision = d;
    }
