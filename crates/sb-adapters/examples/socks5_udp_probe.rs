...
async fn main() -> Result<()> {
    ...
    sb_adapters::socks5_codec::greet_noauth(&mut ctrl).await?;
    let resolve_bound_domain = std::env::var("SB_SOCKS_UDP_RESOLVE_BND")
        .ok()
        .is_some_and(|value| value == "1" || value.eq_ignore_ascii_case("true"));
    let relay = sb_adapters::socks5_codec::udp_associate(
        &mut ctrl,
        "0.0.0.0:0".parse()?,
        resolve_bound_domain,
    )
    .await?;
    ...
    let pkt = sb_adapters::socks5_codec::encode_udp_request(&dns, &q);
    ...
    let (_dst, body) = sb_adapters::socks5_codec::decode_udp_reply(&buf[..m])?;
