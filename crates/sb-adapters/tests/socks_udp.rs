    ...
    use std::sync::Arc;
        ...
        // Test UDP ASSOCIATE
	        ...
	        let udp_conn = match connector.dial_udp(target, opts).await {
		            Ok(c) => c,
		            Err(e) if should_skip(&e) => {
		                eprintln!("skipping socks udp associate test: PermissionDenied creating udp associate");
		                tcp_task.abort();
		                udp_task.abort();
		                return Ok(());
		            }
        ...
        let test_data = b"Hello, SOCKS5 UDP!";
	        let sent = match udp_conn.send_to(test_data).await {
	            Ok(n) => n,
	            Err(e) if should_skip(&e) => {
	                eprintln!("skipping socks udp associate test: PermissionDenied sending udp datagram");
	                tcp_task.abort();
	                udp_task.abort();
	                return Ok(());
	            }
	        ...
	        let received = match udp_conn.recv_from(&mut recv_buf).await {
	            Ok(n) => n,
	            Err(e) if should_skip(&e) => {
	                eprintln!("skipping socks udp associate test: PermissionDenied receiving udp datagram");
	                tcp_task.abort();
	                udp_task.abort();
	                return Ok(());
	            }
