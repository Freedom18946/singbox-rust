...
impl EchConnector {
    ...
    pub fn wrap_tls(&self, server_name: &str) -> EchResult<EchClientHello> {
        ...
        let ech_payload = self.build_ech_payload(
            &encapsulated_key,
            &encrypted_ch,
            ech_config,
        )?;
...
mod tests {
    ...
    fn test_ech_verify_acceptance_empty() {
        ...
        assert!(!connector.verify_ech_acceptance(&empty_server_hello).unwrap());
        
        // Test with very short ServerHello
        let short_server_hello = vec![0x00];
        assert!(!connector.verify_ech_acceptance(&short_server_hello).unwrap());
    }
