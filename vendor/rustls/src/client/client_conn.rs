...
/// A hook that can replace the `session_id` bytes for an outgoing client hello.
///
/// The generator receives the encoded client hello handshake message without the
/// TLS record header. The provided `client_hello` bytes are encoded with a
/// zeroed `session_id` field, matching the bytes that Go ShadowTLS v3 authenticates.
///
/// Implementations must fill the provided `session_id` slice in place.
pub trait SessionIdGenerator: fmt::Debug + Send + Sync {
    /// Generate replacement bytes for the outgoing client hello `session_id`.
...
pub struct ClientConfig {
    ...
    /// Optional hook to replace the outgoing client hello `session_id`.
    ///
    /// When configured, rustls encodes the client hello with a zeroed
    /// `session_id`, passes those bytes to the generator, then emits the
