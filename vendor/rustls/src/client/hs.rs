...
fn maybe_apply_session_id_generator(
    config: &ClientConfig,
    ...
    let Some(generator) = config.session_id_generator.as_ref() else {
        return Ok(());
    };

    let session_id_len = match &mut chp.0 {
        HandshakePayload::ClientHello(client_hello) => {
            let len = client_hello.session_id.len();
            client_hello.session_id = SessionId::zeroed(len);
            len
        }
        _ => return Ok(()),
    };
    ...
    if session_id_len != 32 {
        return Err(Error::General(format!(
            "client hello session_id generator requires a 32-byte session_id, got {} bytes",
            session_id_len
        )));
    }

    let client_hello_bytes = chp.get_encoding();

    if client_hello_bytes.len() < CLIENT_HELLO_SESSION_ID_START + session_id_len
        || client_hello_bytes[0] != HandshakeType::ClientHello.into()
        || client_hello_bytes[CLIENT_HELLO_SESSION_ID_START - 1] != session_id_len as u8
    {
        return Err(Error::General(
            "client hello session_id generator received an unexpected client hello layout"
                .to_owned(),
        ));
    }

    let mut generated_session_id = SessionId::zeroed(session_id_len);
    generator.generate(&client_hello_bytes, generated_session_id.as_mut())?;
    let HandshakeMessagePayload(HandshakePayload::ClientHello(client_hello)) = chp else {
        unreachable!("client hello payload changed while applying session_id generator");
    };
    client_hello.session_id = generated_session_id;
...
fn emit_client_hello_for_retry(
    ...
    maybe_apply_session_id_generator(config, &mut chp)?;
