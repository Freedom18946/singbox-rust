...
- **仍是 Phase 2 / Phase 4 第一波 blocker**：
  - `sb-config` Raw 边界（~~`outbound.rs`~~ ✅ 已完成 Raw/Validated 边界试点 2026-03-26 / `ir/mod.rs` 与 `validator/v2/mod.rs` 的 tail stabilization 已在 2026-04-02 的 WP-30aq 完成：`Credentials`、`Listable<T>` / `StringOrObj<T>` owner 已迁入 `ir/{credentials,value_wrappers}.rs`，shared parse/list helpers + `emit_issue` owner 已迁入 `validator/v2/helpers.rs`，当前两个 `mod.rs` 都只保留 thin compat/re-export facade / `WP-30ar` 又把 DNS Raw owner 与 Raw->Validated bridge 从 `ir/raw.rs` 巨石迁入 `ir/dns_raw.rs`，`ir/raw.rs` 仅保留 compat/re-export + `RawConfigRoot` broader boundary / `WP-30as` 再把 validated/planned seam 收成 staged private facade：`collect_planned_facts` / `validate_with_planned_facts` / `validate_planned_facts`，但 **没有** 引入 generic query API 或 public `RuntimePlan`；**剩余主战场已从 mod.rs / DNS Raw owner 转向“等待真实消费者出现后的 exact accessor / future RuntimePlan”** / ~~`validator/v2` outbound 子域~~ ✅ 已拆分 2026-03-26，且已在 2026-04-01 收口 outbound lowering owner / ~~`validator/v2` route 子域~~ ✅ 已拆分且已在 2026-04-01 收口 route lowering owner / ~~`validator/v2` dns 子域~~ ✅ 已拆分 2026-03-26 / ~~`validator/v2` service 子域~~ ✅ 已拆分 2026-03-26 / ~~`validator/v2` endpoint 子域~~ ✅ 已拆分 2026-03-26 / ~~`validator/v2` facade owner~~ ✅ 已于 2026-04-01 迁移到 `v2/facade.rs`，shared helper seam 也已于 2026-04-02 稳定化）
...
- `crates/sb-config/src/ir/mod.rs` 巨石问题仍未根治，但 owner 收口已继续推进：2026-03-26 完成 endpoint + service 第一刀结构拆分；2026-04-01（WP-30ag/30ah/30ai）继续将 service/DERP、Hysteria2 masquerade、multiplex/brutal owner 下沉到 `ir/service.rs`、`ir/inbound.rs`、`ir/multiplex.rs`；2026-04-02（WP-30aq）再将 `Credentials`、`Listable<T>` / `StringOrObj<T>` owner 下沉到 `ir/credentials.rs` 与 `ir/value_wrappers.rs`，同日 `WP-30ar` 再将 DNS Raw owner 下沉到 `ir/dns_raw.rs`。当前 `ir/mod.rs` 已收至 135 行，主要保留 `experimental` 与 `pub use` compat facade。剩余重点不再是 DNS Raw owner 本身，而是更宽的 raw/validated/planned/normalize consumer seam。
- ~~`crates/sb-config/src/validator/v2.rs` 仍是 5384 LOC 巨石校验器。~~ → ✅ 2026-03-26 已完成 outbound + route + dns + service + endpoint 子域拆分；其后 2026-03-31/2026-04-01 继续把 inbound / route / outbound lowering owner 下沉到对应子模块，并在 2026-04-01 将 facade owner 迁移到 `v2/facade.rs`。2026-04-02（WP-30aq）又把 shared parse/list helpers 与 `emit_issue` owner 迁入 `v2/helpers.rs`。当前 `v2/mod.rs` 已收至 49 行（thin facade + compat re-export + TLS re-export），**validator/v2 第一轮子域拆分与 tail stabilization 已完成；`WP-30ar` 也确认 DNS validation + lowering owner 继续稳定留在 `validator/v2/dns.rs`，剩余主战场转为更宽的 raw/validated/planned 语义边界**。
