...
- **仍是 Phase 2 / Phase 4 第一波 blocker**：
  - `sb-config` Raw 边界（~~`outbound.rs`~~ ✅ 已完成 Raw/Validated 边界试点 2026-03-26 / `ir/mod.rs` / ~~`validator/v2` outbound 子域~~ ✅ 已拆分 2026-03-26 / `validator/v2` 其余子域 dns/route/service/endpoint）
...
- ~~`crates/sb-config/src/validator/v2.rs` 仍是 5384 LOC 巨石校验器。~~ → ✅ 2026-03-26 已完成 outbound 子域拆分（`v2/outbound.rs` 610 行，`v2/mod.rs` 5048 行），dns/route/service/endpoint 子域待拆。
