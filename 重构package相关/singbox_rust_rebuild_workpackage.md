...
- **仍是 Phase 2 / Phase 4 第一波 blocker**：
  - `sb-config` Raw 边界（~~`outbound.rs`~~ ✅ 已完成 Raw/Validated 边界试点 2026-03-26 / `ir/mod.rs` 与 `validator/v2/mod.rs` 的 tail stabilization 已在 2026-04-02 的 WP-30aq 完成：`Credentials`、`Listable<T>` / `StringOrObj<T>` owner 已迁入 `ir/{credentials,value_wrappers}.rs`，shared parse/list helpers + `emit_issue` owner 已迁入 `validator/v2/helpers.rs`，当前两个 `mod.rs` 都只保留 thin compat/re-export facade / `WP-30ar` 又把 DNS Raw owner 与 Raw->Validated bridge 从 `ir/raw.rs` 巨石迁入 `ir/dns_raw.rs`，`ir/raw.rs` 仅保留 compat/re-export + `RawConfigRoot` broader boundary；**剩余主战场已从 mod.rs / DNS Raw owner 转向更宽的 raw/validated/planned/normalize consumer seam** / ~~`validator/v2` outbound 子域~~ ✅ 已拆分 2026-03-26，且已在 2026-04-01 收口 outbound lowering owner / ~~`validator/v2` route 子域~~ ✅ 已拆分且已在 2026-04-01 收口 route lowering owner / ~~`validator/v2` dns 子域~~ ✅ 已拆分 2026-03-26 / ~~`validator/v2` service 子域~~ ✅ 已拆分 2026-03-26 / ~~`validator/v2` endpoint 子域~~ ✅ 已拆分 2026-03-26 / ~~`validator/v2` facade owner~~ ✅ 已于 2026-04-01 迁移到 `v2/facade.rs`，shared helper seam 也已于 2026-04-02 稳定化）
