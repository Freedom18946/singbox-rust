...
- **仍是 Phase 2 / Phase 4 第一波 blocker**：
  - `sb-config` Raw 边界（~~`outbound.rs`~~ ✅ 已完成 Raw/Validated 边界试点 2026-03-26 / `ir/mod.rs`（已完成 endpoint + service 第一刀子模块拆分 2026-03-26，并在 2026-04-01 的 WP-30ag 继续将 service/DERP 专属类型 owner 下沉到 `ir/service.rs`、在同日的 WP-30ah 将 Hysteria2 masquerade owner 下沉到 `ir/inbound.rs`、随后在 WP-30ai 将 `MultiplexOptionsIR` / `BrutalIR` owner 下沉到 `ir/multiplex.rs`；当前 `ir/mod.rs` 主要剩 `Credentials` / `Listable<T>` / `StringOrObj<T>` + experimental / compat 暴露，剩余主战场是 dns IR + raw/validated/planned/normalize 三相边界治理）/ ~~`validator/v2` outbound 子域~~ ✅ 已拆分 2026-03-26，且已在 2026-04-01 收口 outbound lowering owner / ~~`validator/v2` route 子域~~ ✅ 已拆分且已在 2026-04-01 收口 route lowering owner / ~~`validator/v2` dns 子域~~ ✅ 已拆分 2026-03-26 / ~~`validator/v2` service 子域~~ ✅ 已拆分 2026-03-26 / ~~`validator/v2` endpoint 子域~~ ✅ 已拆分 2026-03-26 / ~~`validator/v2` facade owner~~ ✅ 已于 2026-04-01 迁移到 `v2/facade.rs`，`v2/mod.rs` 收至 260 行，仅保留 shared helper + TLS re-export / **validator/v2 第一轮子域拆分与 facade 收口已完成，下一战场仍是 ir/mod.rs 与更大三相边界治理**）
...
- `crates/sb-config/src/ir/mod.rs` 巨石问题仍未根治，但已继续推进 owner 收口：2026-03-26 完成 endpoint + service 第一刀结构拆分；2026-04-01（WP-30ag）继续将 service/DERP 专属类型 owner 下沉到 `ir/service.rs`；同日（WP-30ah）再将 Hysteria2 masquerade owner 下沉到 `ir/inbound.rs`；随后（WP-30ai）将 `MultiplexOptionsIR` / `BrutalIR` owner 下沉到 `ir/multiplex.rs`。当前 `ir/endpoint.rs` 174 行，`ir/service.rs` 694 行（含测试），`ir/multiplex.rs` 199 行（含测试），`ir/mod.rs` 已收至 252 行并主要保留 `Credentials` / `Listable<T>` / `StringOrObj<T>` + experimental / compat 暴露。剩余重点仍是 dns IR 与更大的 raw/validated/planned/normalize 三相边界治理。
- ~~`crates/sb-config/src/validator/v2.rs` 仍是 5384 LOC 巨石校验器。~~ → ✅ 2026-03-26 已完成 outbound + route + dns + service + endpoint 子域拆分；其后 2026-03-31/2026-04-01 继续把 inbound / route / outbound lowering owner 下沉到对应子模块，并在 2026-04-01 将 facade owner 迁移到 `v2/facade.rs`。当前 `v2/outbound.rs` 已是 outbound validation + lowering owner，`v2/route.rs` 已是 route validation + lowering owner，`v2/mod.rs` 已收至 260 行（shared helper + TLS re-export），**validator/v2 第一轮子域拆分与 facade 收口已完成，剩余主战场转为 `ir/mod.rs` 与更大三相边界治理**。
