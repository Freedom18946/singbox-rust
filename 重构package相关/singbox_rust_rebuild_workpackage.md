...
- ~~`crates/sb-config/src/validator/v2.rs` 仍是 5384 LOC 巨石校验器。~~ → ✅ 2026-03-26 已完成 outbound + route + dns + service 子域拆分（`v2/outbound.rs` 610 行 + `v2/route.rs` 362 行 + `v2/dns.rs` 221 行 + `v2/service.rs` 135 行，`v2/mod.rs` 4710 行），endpoint 子域待拆。
