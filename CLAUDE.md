...
- **当前现状**：严格门禁 **exit 0**（V7 537 断言，0 违规）。此前登记的脚本陈旧目标漂移
  （`validator/v2.rs` 已拆成 `validator/v2/` 等失配）已于 2026-06-06 修复。实时门禁状态
  以 `active_context.md` 为准。
...
```
agents-only/
├── active_context.md            # S: 当前状态唯一权威（≤300行）
