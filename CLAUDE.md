...
- **阻塞/待外部条件**:
  - `cargo fmt --all -- --check` 失败（仓库既有格式漂移，非 L17 变更引入）。
  - `cargo clippy --workspace --all-features --all-targets -- -D warnings` 失败（`sb-tls`/`sb-config` 既有 lint）。
  - `cargo test --workspace` 失败（`-p sb-core --test udp_balancer` 中 `balancer_socks5_ok`）。
  - Docker daemon 当前不可用，无法本机完成镜像构建/运行实测。
  - Canary 24h 短跑未在本次会话内完成（框架与命令已就绪）。
...
| `cargo fmt --all -- --check` | ❌（既有格式漂移） |
| `cargo clippy --workspace --all-features --all-targets -- -D warnings` | ❌（既有 lint） |
| `cargo test --workspace` | ❌（`sb-core`/`udp_balancer::balancer_socks5_ok`） |
| `cargo test -p interop-lab` | ✅ 27 passed |
| `check-boundaries.sh` | ✅ exit 0 |
| `docker build` | ⚠️ 环境阻塞（daemon 未启动） |
| interop-lab cases | 81 total (72 strict, 8 env_limited, 1 smoke) |
