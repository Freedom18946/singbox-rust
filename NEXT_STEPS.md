...
- [P1][PX-002][DONE 2025-12-29] Enforce Go-style unknown-field errors during config load (DisallowUnknownFields) in `crates/sb-config/src/lib.rs::load`. Verified with `test_schema_field_allowed` (unknown fields rejected, `$schema` allowed).
- [P1][PX-002][DONE 2025-12-29] Preserve inbound tag/name in IR and validate uniqueness across inbounds/outbounds/endpoints in `crates/sb-config/src/ir/mod.rs` + `crates/sb-config/src/validator/v2.rs` + `crates/sb-config/src/lib.rs::validate`. Verified with `test_inbound_tag_parsed`, `test_duplicate_inbound_tag_rejected`, `test_inbound_outbound_tag_conflict_rejected`.
- [P1][PX-002][DONE 2025-12-29] Align log options fields (disabled/output) with Go in `crates/sb-config/src/ir/mod.rs` + `crates/sb-config/src/validator/v2.rs`. Verified with `test_log_disabled_output_parsed`.
- [P2][PX-002][DONE 2025-12-29] Allow `$schema` under strict validation to match Go’s optional field in `crates/sb-config/src/validator/v2.rs`. Verified with `test_schema_field_allowed`.
...
- [P0][PX-003][DONE 2025-12-30] Implement Go rule actions and logical rules (route/direct/reject/hijack/sniff/resolve + logical mode) in `crates/sb-config/src/validator/v2.rs` + `crates/sb-config/src/ir/mod.rs`. Verified with `test_rule_action_reject_parsed`, `test_rule_action_hijack_with_override`, `test_logical_rule_and_mode`.
- [P0][PX-003][DONE 2025-12-29] Fix rule parsing bug mapping `domain_suffix` into `domain` in `crates/sb-config/src/lib.rs::rule_from_ir`. Verified with `test_domain_suffix_mapping_from_ir`.
- [P0][PX-004][DONE 2025-12-30] Implement Go-style DNSRouter + DNSTransportManager (rule evaluation, transport tags, fakeip constraints, start order) in `crates/sb-core/src/dns/*`. Verified with `test_transport_manager_*`, `test_query_context_builder`, `test_null_router_fails`.
- [P0][PX-004][DONE 2025-12-30] Align DNS client behavior (cache/TTL/negative TTL/EDNS0 subnet/response reject/RDRC) in `crates/sb-core/src/dns/client.rs`. Verified with `test_ttl_config_*`, `test_ttl_clamping_*`, `test_negative_cache_ttl`.
- [P0][PX-005][DONE 2025-12-30] Implement Go-style runtime router Phase 1-3: Extended `Decision` enum (RejectDrop/Hijack/Sniff/Resolve), created `RouteConnection` interface (`RouteResult`/`ConnectionRouter`/`DirectRouter`), integrated FakeIP reverse lookup in `RouterHandle`. Fixed 12+ inbound handler Decision matches in `sb-adapters`. Verified with `cargo check -p sb-core --lib` and `cargo check -p sb-adapters --lib`.
- [P0][PX-005][DONE 2025-12-30] Implement ConnectionManager parity (dialer network strategy/fallback, TLS fragmentation stub, UDP NAT stub, UDP timeouts, handshake error semantics) in `crates/sb-core/src/router/conn.rs`. Added `RouteConnectionManager` with `new_connection()`/`new_packet_connection()` methods, `dial_serial_network()` for network strategy, `port_protocol()`/`protocol_timeout()` for UDP timeouts. Verified with `cargo check -p sb-core --lib` and `cargo check -p sb-adapters --lib`.
- [P0][PX-005][DONE 2025-12-30] Full integration: Wire RouteConnectionManager into adapter bridge. Added `connection_manager` field to `AdapterInboundContext`, created and passed `RouteConnectionManager` in `build_bridge()` (feature-gated behind `router`). Verified with `cargo check -p sb-core --lib` and `cargo check -p sb-adapters --lib`.
- [P0][PX-006][DONE 2025-12-30] Implement Go-style lifecycle stages (Initialize/Start/PostStart/Started) for inbound/outbound/service managers. Added `Lifecycle` trait to `service.rs`, created `InboundAdapter`/`OutboundAdapter` traits with `start_all()`/`close_all()` methods. Verified with `cargo test -p sb-core -- manager` (14 tests pass).
- [P0][PX-006][DONE 2025-12-30] Align outbound manager defaults/dependencies. Added `dependencies` field with `add_dependency()`, `get_startup_order()` (Kahn's topological sort with cycle detection), and `ensure_fallback_direct()` to `OutboundManager`. Verified with `cargo check -p sb-core --lib`.
- [P0][PX-007][DONE 2025-12-30] Implement ConnectionRouter for RouterHandle. Added `route_connection()`, `route_packet()`, `pre_match()` async methods wrapping `decide()`, with `decision_to_route_result()` helper. Verified with `cargo check -p sb-core --lib`.
- [P0][PX-008][DONE 2025-12-30] Add DnsRouter to AdapterInboundContext. Added `dns_router: Option<Arc<dyn DnsRouter>>` field to `AdapterInboundContext` in `registry.rs` and wired in both router/non-router `build_bridge()`. Verified with `cargo check -p sb-core --lib`.
- [P0][PX-010][DONE 2025-12-30] Clash API service already implemented with 45+ handlers (get_proxies, select_proxy, get_connections, get_rules, get_configs, get_proxy_providers, flush_dns_cache, etc.). Fixed `infer_proxy_type` match pattern for feature-gated variants. Verified with `cargo check -p sb-api`.
- [P0][PX-011][DONE 2025-12-30] SSMAPI already implemented with `SsmapiService` (per-endpoint `servers` map, `UserManager`, per-endpoint cache/load/save). Has `create_inbound_routes()` for per-endpoint API. Verified with `cargo check -p sb-core --lib`.
- [P0][PX-011][DONE 2025-12-30] Traffic tracking implemented with `TrafficManager` (record_uplink/downlink, TCP/UDP sessions), implements `TrafficTracker` trait for SS adapter integration. Verified with `cargo check -p sb-core --lib`.
- [P0][PX-012][DONE 2025-12-30] V2Ray StatsService already implemented with HTTP endpoints matching gRPC paths (GetStats/QueryStats/GetSysStats). Has `StatsManager` with counters, init_standard_counters, update_traffic. Verified in `v2ray_api.rs` (509 lines).
- [P0][PX-012][DONE 2025-12-30] ConnectionTracker integrated via `StatsManager` with standard V2Ray counter naming (`inbound>>>tag>>>traffic>>>uplink/downlink`). Traffic updated via `update_traffic()`. Verified in `v2ray_api.rs`.
- [P0][PX-013][DONE 2025-12-30] RDRC cache already implemented in `CacheFileService` (473 lines) with `RdrcEntry` struct, `put_rdrc()`, `get_rdrc()` with TTL expiry check, `flush()` for async save, store_rdrc config flag. Verified via outline.
- [P0][PX-015][DONE 2025-12-30] Resolved service DNS handling already implemented in `resolved_impl.rs` (686 lines) with `ResolvedService`, `handle_dns_query()`, `spawn_dns_server()` (TCP/UDP), `DnsResolver` integration. Verified via outline.
