...
fn bench_trojan_throughput(c: &mut Criterion) {
    ...
    let counters = Arc::new(E2eCounters::default());
    ...
    rt.block_on(async {
        if !wait_for_trojan_ready(trojan_addr, echo_addr, Duration::from_secs(3)).await {
            tracing::warn!(%trojan_addr, "trojan server readiness probe timed out");
        }
    });
            ...
            b.to_async(&rt).iter_custom(move |iters| {
                ...
                async move {
                    ...
                    let Some(mut stream) = connect().await else {
                        counters.dropped_iterations.fetch_add(1, Ordering::Relaxed);
                            ...
                            .await
                        {
                            ...
                            Ok(Err(_)) => {
                                ...
                                if let Some(new_stream) = connect().await {
                                    ...
                                    counters.reconnect_failures.fetch_add(1, Ordering::Relaxed);
                                    break;
                                }
                                continue;
                            }
    ...
    group.finish();
    eprintln!(
        "trojan_throughput summary: dropped_iterations={} reconnect_failures={} write_failures={} write_timeouts={} read_failures={} read_timeouts={}",
        counters.dropped_iterations.load(Ordering::Relaxed),
        counters.reconnect_failures.load(Ordering::Relaxed),
        counters.write_failures.load(Ordering::Relaxed),
        counters.write_timeouts.load(Ordering::Relaxed),
        counters.read_failures.load(Ordering::Relaxed),
        counters.read_timeouts.load(Ordering::Relaxed)
    );
}
...
fn bench_trojan_handshake_overhead(c: &mut Criterion) {
    ...
    rt.block_on(async {
        if !wait_for_trojan_ready(trojan_addr, echo_addr, Duration::from_secs(3)).await {
            tracing::warn!(%trojan_addr, "trojan handshake server readiness probe timed out");
        }
    });
