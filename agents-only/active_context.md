...
### 🆕 MIG-02 正式关闭 + 验收通过（2026-03-07, ACCEPTED）

**关闭标志**：两轮独立全量审计通过 + 大验收 Step 0-5 全绿，生产源码零隐式直连回退。
...
| Trojan/VLess/VMess/SS/STls/AnyTls/Redirect/TProxy inbound | rules_global None → Err | **W200** |
| SOCKS5 UDP enhanced | rules_global None → Reject | **W201** |
| TUN macOS | process_router None → Err | **W201** |
| Tailscale WireGuard/Socks5/Managed modes | 无 self.direct 回退 → Err | **W202** |
...
- V7 断言：471 → 476（+5 新断言）
- ~~全项目 env-var silent parse fallback 审计完成~~ → **修正**：wave193-198 发现 sb-core/sb-transport/sb-adapters 层仍有 21 个遗漏 env vars，已在 wave193-198 中全部收口
...
### L21 wave#55-160 历史摘要（已归档）

> 以下 106 波的逐波快照已归档到 `log.md` 和 `reports/l21/artifacts/`。仅保留里程碑汇总：

| 波次范围 | 时间 | 主要收口内容 | V7 断言 |
|----------|------|-------------|---------|
| wave#55-122 | 03-05 ~ 03-06 | sb-core tests `default=direct` 清零 + sb-adapters tests 清零 + app tests 清零 | 150 → 304 |
| wave#123-130 | 03-06 | router/mod silent direct fallback → unresolved; socks/shadowsocks/hysteria2 NeedFallback 去 direct | 306 → 322 |
| wave#131-140 | 03-06 | config validator/bridge silent parse; Hysteria2 inbound TLS fallback; HTTP/Mixed inbound → sb-adapters | 324 → 346 |
| wave#141-145 | 03-06 | router engine geoip/geosite env + explain-index 去 direct; tailscale/diag unused import cleanup | 348 → 358 |
| wave#146-154 | 03-06 | router engine geoip cache/TTL; vmess/vless server parse; register.rs listen_addr 收口 | 358 → 368 |
| wave#155-160 | 03-06 | router rules env (MAX/HOT_RELOAD_MS/JITTER_MS/BACKOFF_MAX_MS/MAX_DEPTH/INCLUDE_DEPTH) | 370 → 380 |

> 逐波证据文件位于 `reports/l21/artifacts/waveXX_*.txt`，V7 回归负样例均已验证。
