...
- 状态：完成（`crates/sb-core/src/router/mod.rs` 已完成 `SB_ROUTER_RULES_INCLUDE_DEPTH` parse-failure 收口并同步升级 V7）。
- 本轮落地：
  1. `crates/sb-core/src/router/mod.rs`：新增 `parse_router_rules_include_depth_env(...)` 与 `router_rules_include_depth_from_env()`，include 递归深度 env 不再静默吞掉 invalid `SB_ROUTER_RULES_INCLUDE_DEPTH`，改为显式告警后回退到默认 `4`
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.157-wave160-v1`（380 assertions）
  3. `wave160_v7_regression_block.txt`：恢复旧 `v.parse::<usize>().ok()` 后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
...
- 状态：完成（`crates/sb-core/src/router/mod.rs` 已完成 `SB_ROUTER_RULES_BACKOFF_MAX_MS` parse-failure 收口并同步升级 V7）。
- 本轮落地：
  1. `crates/sb-core/src/router/mod.rs`：新增 `parse_router_rules_backoff_max_ms_env(...)` 与 `router_rules_backoff_max_ms_from_env()`，热重载 backoff cap env 不再静默吞掉 invalid `SB_ROUTER_RULES_BACKOFF_MAX_MS`，改为显式告警后回退到默认 `30000`
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.155-wave158-v1`（376 assertions）
  3. `wave158_v7_regression_block.txt`：恢复旧 `v.parse::<u64>().ok()` 后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
...
### 🆕 L21 wave#157 推进快照（2026-03-06 19:35）

- 状态：完成（`crates/sb-core/src/router/mod.rs` 已完成 `SB_ROUTER_RULES_JITTER_MS` parse-failure 收口并同步升级 V7）。
- 本轮落地：
  1. `crates/sb-core/src/router/mod.rs`：新增 `parse_router_rules_jitter_ms_env(...)` 与 `router_rules_jitter_ms_from_env()`，热重载 jitter env 不再静默吞掉 invalid `SB_ROUTER_RULES_JITTER_MS`，改为显式告警后回退到默认 `0`
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.154-wave157-v1`（374 assertions）
  3. `wave157_v7_regression_block.txt`：恢复旧 `v.parse::<u64>().ok()` 后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
...
- 状态：完成（`crates/sb-core/src/router/mod.rs` 已完成 `SB_ROUTER_RULES_MAX` parse-failure 收口并同步升级 V7）。
- 本轮落地：
  1. `crates/sb-core/src/router/mod.rs`：新增 `parse_router_rules_max_env(...)` 与 `router_rules_max_from_env()`，热重载 / env 初始化 / shared index 路径不再静默吞掉 invalid `SB_ROUTER_RULES_MAX`，改为显式告警后回退到默认 `8192`
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.152-wave155-v1`（370 assertions）
  3. `wave155_v7_regression_block.txt`：恢复旧 `v.parse().ok()` 后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
...
- 状态：完成（`crates/sb-adapters/src/register.rs` 已完成 `vmess` outbound server socket parse 收口并同步升级 V7）。
- 本轮落地：
  1. `crates/sb-adapters/src/register.rs`：新增 `parse_required_outbound_socket_addr(...)`，`build_vmess_outbound(...)` 不再静默吞掉 invalid `server:port`，改为显式 invalid-config connector
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.149-wave152-v1`（364 assertions）
  3. `wave152_v7_regression_block.txt`：恢复旧 `parse::<SocketAddr>().ok()?` 后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
...
### 🆕 L21 wave#151 推进快照（2026-03-06 18:49）

- 状态：完成（`crates/sb-core/src/router/engine.rs` 已完成 geoip `SB_GEOIP_TTL` parse-failure 收口并同步升级 V7）。
- 本轮落地：
  1. `crates/sb-core/src/router/engine.rs`：新增 `parse_geoip_ttl_env(...)`，`init_geoip_if_env()` 不再静默吞掉 invalid `SB_GEOIP_TTL`，改为显式告警后回退到默认 `600s`
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.148-wave151-v1`（362 assertions）
  3. `wave151_v7_regression_block.txt`：恢复旧 `humantime::parse_duration(...).ok()` 后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
...
### 🆕 L21 wave#150 推进快照（2026-03-06 18:41）

- 状态：完成（`crates/sb-core/src/router/engine.rs` 已完成 geoip `SB_GEOIP_CACHE` parse-failure 收口并同步升级 V7）。
- 本轮落地：
  1. `crates/sb-core/src/router/engine.rs`：新增 `parse_geoip_cache_cap_env(...)`，`init_geoip_if_env()` 不再静默吞掉 invalid `SB_GEOIP_CACHE`，改为显式告警后回退到默认 `8192`
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.147-wave150-v1`（360 assertions）
  3. `wave150_v7_regression_block.txt`：恢复旧 `v.parse().ok()` 后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
...
### 🆕 L21 wave#149 推进快照（2026-03-06 18:33）

- 状态：完成（`crates/sb-core/src/router/engine.rs` 已完成 router `SB_ROUTER_UDP_RULES` parse-failure 收口并同步升级 V7）。
- 本轮落地：
  1. `crates/sb-core/src/router/engine.rs`：新增 `parse_udp_rules_index(...)`，`udp_rules_index_from_env()` 不再静默吞掉 invalid `SB_ROUTER_UDP_RULES`，改为显式告警后拒绝加载该 env rules 索引
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.146-wave149-v1`（358 assertions）
  3. `wave149_v7_regression_block.txt`：恢复旧 `.ok()?` 后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
...
### 🆕 L21 wave#148 推进快照（2026-03-06 18:23）

- 状态：完成（`crates/sb-adapters/src/register.rs` 已完成 `legacy_shadowsocksr` outbound invalid TryFrom config 收口并同步升级 V7）。
- 本轮落地：
  1. `crates/sb-adapters/src/register.rs`：`build_shadowsocksr_outbound(...)` 不再静默吞掉 `ShadowsocksROutbound::try_from(ir)` 失败，改为复用 `invalid_outbound_config_reason(...)` 并返回显式 invalid-config connector
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.145-wave148-v1`（356 assertions）
  3. `wave148_v7_regression_block.txt`：恢复旧 `.ok()?` 后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
...
- 状态：完成（`crates/sb-adapters/src/register.rs` 已完成 dns outbound `server` IP 配置解析收口并同步升级 V7）。
- 本轮落地：
  1. `crates/sb-adapters/src/register.rs`：新增 `parse_required_outbound_ip_addr(...)`，`build_dns_outbound(...)` 不再静默吞掉 invalid `server` IP，改为显式 invalid-config connector；缺失 `server` 仍保持 `None`
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.143-wave146-v1`（352 assertions）
  3. `wave146_v7_regression_block.txt`：恢复旧 `parse().ok()` 后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
...
### 🆕 L21 wave#145 推进快照（2026-03-06 17:56）

- 状态：完成（`crates/sb-adapters/src/register.rs` 已完成 tuic outbound `uuid` 配置解析收口并同步升级 V7）。
- 本轮落地：
  1. `crates/sb-adapters/src/register.rs`：`build_tuic_outbound(...)` 切到 `parse_required_outbound_uuid(...)`，使 invalid `uuid` 不再静默吞掉，改为显式 invalid-config connector；缺失 `uuid` 仍保持 `None`
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.142-wave145-v1`（350 assertions）
  3. `wave145_v7_regression_block.txt`：恢复旧 `parse_str(...).ok()?` 后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
...
### 🆕 L21 wave#144 推进快照（2026-03-06 17:53）

- 状态：完成（`crates/sb-adapters/src/register.rs` 已完成 vless outbound `uuid` 配置解析收口并同步升级 V7）。
- 本轮落地：
  1. `crates/sb-adapters/src/register.rs`：`build_vless_outbound(...)` 切到 `parse_required_outbound_uuid(...)`，使 invalid `uuid` 不再静默吞掉，改为显式 invalid-config connector；缺失 `uuid` 仍保持 `None`
  2. helper 测试移到无条件 `#[cfg(test)]` 模块，确保默认特性下也会编译
  3. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.141-wave144-v1`（348 assertions）
  4. `wave144_v7_regression_block.txt`：恢复旧 `parse_str(...).ok()?` 后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
...
### 🆕 L21 wave#143 推进快照（2026-03-06 17:47）

- 状态：完成（`crates/sb-adapters/src/register.rs` 已完成 vmess outbound `uuid` 配置解析收口并同步升级 V7）。
- 本轮落地：
  1. `crates/sb-adapters/src/register.rs`：新增 `parse_required_outbound_uuid(...)` 与 `InvalidConfigConnector`，使 `build_vmess_outbound(...)` 不再静默吞掉 invalid `uuid`，改为显式 invalid-config connector；缺失 `uuid` 仍保持 `None`
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.140-wave143-v1`（346 assertions）
  3. `wave143_v7_regression_block.txt`：恢复旧 `parse_str(...).ok()?` 后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
...
- 状态：完成（`crates/sb-core/src/adapter/bridge.rs` 已完成 outbound `connect_timeout` 配置解析收口并同步升级 V7）。
- 本轮落地：
  1. `crates/sb-core/src/adapter/bridge.rs`：新增 `parse_optional_outbound_duration(...)`，使 `to_outbound_param(...)` 不再静默吞掉 invalid `connect_timeout`，改为显式报错；outbound 组装路径统一拒绝构建该 adapter
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.138-wave141-v1`（342 assertions）
  3. `wave141_v7_regression_block.txt`：恢复旧 `parse_duration(...).ok()` 后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
...
### 🆕 L21 wave#140 推进快照（2026-03-06 17:29）

- 状态：完成（`crates/sb-core/src/adapter/bridge.rs` 已完成 inbound `udp_timeout` 配置解析收口并同步升级 V7）。
- 本轮落地：
  1. `crates/sb-core/src/adapter/bridge.rs`：新增 `parse_optional_inbound_duration(...)`，使 `to_inbound_param(...)` 不再静默吞掉 invalid `udp_timeout`，改为显式报错；bridge 组装路径统一拒绝构建该 inbound adapter
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.137-wave140-v1`（340 assertions）
  3. `wave140_v7_regression_block.txt`：移除 helper 调用后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
...
### 🆕 L21 wave#139 推进快照（2026-03-06 17:23）

- 状态：完成（`app/src/inbound_starter.rs` 已完成 vmess `uuid` 配置解析收口并同步升级 V7）。
- 本轮落地：
  1. `app/src/inbound_starter.rs`：`start_vmess_inbound(...)` 切到 `parse_optional_inbound_uuid(...)`，使 invalid `uuid` 不再静默吞掉，改为显式报错并拒绝启动；缺失 `uuid` 仍保持 skip
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.136-wave139-v1`（338 assertions）
  3. `wave139_v7_regression_block.txt`：恢复旧 `parse_str(...).ok()` 后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
...
### 🆕 L21 wave#138 推进快照（2026-03-06 17:20）

- 状态：完成（`app/src/inbound_starter.rs` 已完成 vless `uuid` 配置解析收口并同步升级 V7）。
- 本轮落地：
  1. `app/src/inbound_starter.rs`：新增 `parse_optional_inbound_uuid(...)`，使 `start_vless_inbound(...)` 不再静默吞掉 invalid `uuid`，改为显式报错并拒绝启动；缺失 `uuid` 仍保持 skip
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.135-wave138-v1`（336 assertions）
  3. `wave138_v7_regression_block.txt`：恢复旧 `parse_str(...).ok()` 后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
...
### 🆕 L21 wave#137 推进快照（2026-03-06 17:16）

- 状态：完成（`app/src/inbound_starter.rs` 已完成 mixed `udp_timeout` 配置解析收口并同步升级 V7）。
- 本轮落地：
  1. `app/src/inbound_starter.rs`：`start_mixed_inbound(...)` 切到 `parse_optional_inbound_duration(...)`，不再静默吞掉无效 `udp_timeout`，改为显式报错并拒绝启动
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.134-wave137-v1`（334 assertions）
  3. `wave137_v7_regression_block.txt`：恢复旧 `parse_duration(...).ok()` 后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
...
### 🆕 L21 wave#136 推进快照（2026-03-06 17:09）

- 状态：完成（`app/src/inbound_starter.rs` 已完成 socks `udp_timeout` 配置解析收口并同步升级 V7）。
- 本轮落地：
  1. `app/src/inbound_starter.rs`：新增 `parse_optional_inbound_duration(...)`，使 `start_socks_inbound(...)` 不再静默吞掉无效 `udp_timeout`，改为显式报错并拒绝启动
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.133-wave136-v1`（332 assertions）
  3. `wave136_v7_regression_block.txt`：恢复旧 `parse_duration(...).ok()` 后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
...
- 状态：完成（`app/src/inbound_starter.rs` 已完成 vmess fallback 配置解析收口并同步升级 V7）。
- 本轮落地：
  1. `app/src/inbound_starter.rs`：`start_vmess_inbound(...)` 切到 `parse_optional_inbound_fallback_addr(...)` 与 `parse_inbound_fallback_for_alpn(...)`，不再静默吞掉无效 `fallback` / `fallback_for_alpn`，改为显式报错并拒绝启动
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.130-wave133-v1`（326 assertions）
  3. `wave133_v7_regression_block.txt`：恢复旧 `parse(...).ok()` 后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
...
- 状态：完成（`crates/sb-adapters/src/inbound/tuic.rs` 已完成本波收口并同步升级 V7）。
- 本轮落地：
  1. `crates/sb-adapters/src/inbound/tuic.rs`：新增 `decision_from_route_target(...)`，使 `OutboundKind::Socks/Http/Naive/Hysteria2` 等 kind 路由目标不再默默记为 `Decision::Direct`，统一改为显式 proxy tag
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.128-wave131-v1`（322 assertions）
  3. `wave131_v7_regression_block.txt`：恢复旧 `_ => Decision::Direct` 后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
...
- 状态：完成（`crates/sb-adapters/src/inbound/naive.rs` 已完成本波收口并同步升级 V7）。
- 本轮落地：
  1. `crates/sb-adapters/src/inbound/naive.rs`：新增 `decision_from_route_target(...)`，使 `OutboundKind::Socks/Http/Naive/Hysteria2` 等 kind 路由目标不再默默记为 `Decision::Direct`，统一改为显式 proxy tag
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.126-wave129-v1`（318 assertions）
  3. `wave129_v7_regression_block.txt`：恢复旧 `_ => Decision::Direct` 后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
...
- 状态：完成（`crates/sb-core/src/router/json_bridge.rs` 已完成本波收口并同步升级 V7）。
- 本轮落地：
  1. `crates/sb-core/src/router/json_bridge.rs`：缺失 `outbound` 不再默认 `Decision::Direct`，改为显式 `unresolved` marker
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.124-wave127-v1`（314 assertions）
  3. `wave127_v7_regression_block.txt`：恢复 `unwrap_or(Decision::Direct)` 后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
  - `wave127_wp1_app_tests_check.txt` PASS
  - `wave127_wp1_sb_core_check.txt` PASS
  - `wave127_sb_core_router_json_tests_check.txt` PASS
...
- 状态：完成（`crates/sb-core/tests/geoip_rules.rs` 已完成本波收口并同步升级 V7）。
- 本轮落地：
  1. `crates/sb-core/tests/geoip_rules.rs`：geoip_rules 注释样例 default 去 silent direct fallback
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.117-wave120-v1`（300 assertions）
  3. `wave120_v7_regression_block.txt`：注入回流样例后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
...
### 🆕 L21 wave#119 推进快照（2026-03-06 04:40）

- 状态：完成（`crates/sb-core/tests/router_cidr4.rs` 已完成本波收口并同步升级 V7）。
- 本轮落地：
  1. `crates/sb-core/tests/router_cidr4.rs`：router_cidr4 测试样例 default 去 silent direct fallback
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.116-wave119-v1`（298 assertions）
  3. `wave119_v7_regression_block.txt`：注入回流样例后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
...
- 状态：完成（`crates/sb-core/tests/router_ipversion_matching.rs` 已完成本波收口并同步升级 V7）。
- 本轮落地：
  1. `crates/sb-core/tests/router_ipversion_matching.rs`：router_ipversion_matching 测试样例 default 去 silent direct fallback
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.114-wave117-v1`（294 assertions）
  3. `wave117_v7_regression_block.txt`：注入回流样例后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
...
### 🆕 L21 wave#116 推进快照（2026-03-06 04:20）

- 状态：完成（`crates/sb-core/tests/router_query_type_matching.rs` 已完成本波收口并同步升级 V7）。
- 本轮落地：
  1. `crates/sb-core/tests/router_query_type_matching.rs`：router_query_type_matching 测试样例 default 去 silent direct fallback
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.113-wave116-v1`（292 assertions）
  3. `wave116_v7_regression_block.txt`：注入回流样例后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
...
### 🆕 L21 wave#115 推进快照（2026-03-06 04:19）

- 状态：完成（`crates/sb-core/tests/router_rules_port_transport.rs` 已完成本波收口并同步升级 V7）。
- 本轮落地：
  1. `crates/sb-core/tests/router_rules_port_transport.rs`：router_rules_port_transport 测试样例 default 去 silent direct fallback
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.112-wave115-v1`（290 assertions）
  3. `wave115_v7_regression_block.txt`：注入回流样例后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
...
### 🆕 L21 wave#114 推进快照（2026-03-06 04:18）

- 状态：完成（`crates/sb-core/tests/router_udp_rules.rs` 已完成本波收口并同步升级 V7）。
- 本轮落地：
  1. `crates/sb-core/tests/router_udp_rules.rs`：router_udp_rules 测试样例 default 去 silent direct fallback
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.111-wave114-v1`（288 assertions）
  3. `wave114_v7_regression_block.txt`：注入回流样例后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
...
### 🆕 L21 wave#113 推进快照（2026-03-06 04:18）

- 状态：完成（`crates/sb-core/tests/router_rules_include_cycle.rs` 已完成本波收口并同步升级 V7）。
- 本轮落地：
  1. `crates/sb-core/tests/router_rules_include_cycle.rs`：router_rules_include_cycle 测试样例 default 去 silent direct fallback
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.110-wave113-v1`（286 assertions）
  3. `wave113_v7_regression_block.txt`：注入回流样例后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
...
### 🆕 L21 wave#112 推进快照（2026-03-06 04:17）

- 状态：完成（`crates/sb-core/tests/router_process_rules_integration.rs` 已完成本波收口并同步升级 V7）。
- 本轮落地：
  1. `crates/sb-core/tests/router_process_rules_integration.rs`：router_process_rules_integration 测试样例 default 去 silent direct fallback
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.109-wave112-v1`（284 assertions）
  3. `wave112_v7_regression_block.txt`：注入回流样例后 `--v7-only` 失败，`exit_code=1`
- 最小验证：
...
- 状态：完成（ 已完成本波收口并同步升级 V7）。
- 本轮落地：
  1. ：router_priority 测试样例 default 去 silent direct fallback
  2.  升级到 （280 assertions）
  3. ：注入回流样例后  失败，
- 最小验证：
  -  PASS
  -  PASS
...
- 状态：（wave#108 已完成 router_cache 两处测试样例中的 fallback 从  调整为 ，去除示例中的 silent direct fallback 字面量 + V7 断言升级）
- 本轮落地：
  1. ：router_cache 两处测试样例中的 fallback 从  调整为 ，去除示例中的 silent direct fallback 字面量。
  2.  升级到 （276 assertions），新增 W108-01~W108-02。
  3. 回流阻断证据：（将  注入回  后  失败，）。
- 最小验证：
  - ：PASS（）
  - ：PASS（）
  - === 依赖边界检查 (2026-03-06 03:55) ===
...
── V7: L20 migration assertions ──
  INFO: assertion version: l21.107-wave110-v1
...
### 🆕 L21 wave#110 推进快照（2026-03-06 03:55)

- 状态：`MIG-02 hardening`（wave#110 已完成 router_priority 测试样例 default 去 silent direct fallback + V7 断言升级）
- 本轮落地：
  1. `crates/sb-core/tests/router_priority.rs`：router_priority 三处测试样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，去除示例中的 silent direct fallback 字面量。
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.107-wave110-v1`（280 assertions），新增 W110-01~W110-02。
  3. 回流阻断证据：`reports/l21/artifacts/wave110_v7_regression_block.txt`（将 `default=unresolved` 注入回 `default=direct` 后 `--v7-only` 失败，`exit_code=1`）。
- 最小验证：
  - `cargo check -p app --tests`：PASS（`wave110_wp1_app_tests_check.txt`）
  - `cargo check -p sb-core`：PASS（`wave110_wp1_sb_core_check.txt`）
  - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`V7 PASS (280 assertions)`）
  - `bash -n scripts/l18/gui_real_cert.sh`：PASS（`wave110_gui_static_syntax_check.txt`）
...
- 状态：`MIG-02 hardening`（wave#108 已完成 router_cache 测试样例 default 去 silent direct fallback + V7 断言升级）
- 本轮落地：
  1. `crates/sb-core/tests/router_cache.rs`：router_cache 两处测试样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，去除示例中的 silent direct fallback 字面量。
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.105-wave108-v1`（276 assertions），新增 W108-01~W108-02。
  3. 回流阻断证据：`reports/l21/artifacts/wave108_v7_regression_block.txt`（将 `default=unresolved` 注入回 `default=direct` 后 `--v7-only` 失败，`exit_code=1`）。
- 最小验证：
  - `cargo check -p app --tests`：PASS（`wave108_wp1_app_tests_check.txt`）
  - `cargo check -p sb-core`：PASS（`wave108_wp1_sb_core_check.txt`）
  - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`V7 PASS (276 assertions)`）
  - `bash -n scripts/l18/gui_real_cert.sh`：PASS（`wave108_gui_static_syntax_check.txt`）

### 🆕 L21 wave#107 推进快照（2026-03-06 03:53）

- 状态：（wave#107 已完成 router_explain 测试样例中的 fallback 从  调整为 ，去除示例中的 silent direct fallback 字面量 + V7 断言升级）
- 本轮落地：
  1. ：router_explain 测试样例中的 fallback 从  调整为 ，去除示例中的 silent direct fallback 字面量。
  2.  升级到 （274 assertions），新增 W107-01~W107-02。
  3. 回流阻断证据：（将  注入回  后  失败，）。
- 最小验证：
  - ：PASS（）
  - ：PASS（）
...
- 状态：`MIG-02 hardening`（wave#107 已完成 router_explain 测试样例 default 去 silent direct fallback + V7 断言升级）
- 本轮落地：
  1. `crates/sb-core/tests/router_explain.rs`：router_explain 测试样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，去除示例中的 silent direct fallback 字面量。
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.104-wave107-v1`（274 assertions），新增 W107-01~W107-02。
  3. 回流阻断证据：`reports/l21/artifacts/wave107_v7_regression_block.txt`（将 `default=unresolved` 注入回 `default=direct` 后 `--v7-only` 失败，`exit_code=1`）。
- 最小验证：
  - `cargo check -p app --tests`：PASS（`wave107_wp1_app_tests_check.txt`）
  - `cargo check -p sb-core`：PASS（`wave107_wp1_sb_core_check.txt`）
  - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`V7 PASS (274 assertions)`）
  - `bash -n scripts/l18/gui_real_cert.sh`：PASS（`wave107_gui_static_syntax_check.txt`）

### 🆕 L21 wave#106 推进快照（2026-03-06 03:53)

- 状态：`MIG-02 hardening`（wave#106 已完成 router_rules_port_range 测试样例 default 去 silent direct fallback + V7 断言升级）
- 本轮落地：
  1. `crates/sb-core/tests/router_rules_port_range.rs`：router_rules_port_range 测试样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，并将区间外默认断言更新为 `unresolved`，去除示例中的 silent direct fallback 字面量。
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.103-wave106-v1`（272 assertions），新增 W106-01~W106-02。
  3. 回流阻断证据：`reports/l21/artifacts/wave106_v7_regression_block.txt`（将 `default=unresolved` 注入回 `default=direct` 后 `--v7-only` 失败，`exit_code=1`）。
- 最小验证：
  - `cargo check -p app --tests`：PASS（`wave106_wp1_app_tests_check.txt`）
  - `cargo check -p sb-core`：PASS（`wave106_wp1_sb_core_check.txt`）
  - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`V7 PASS (272 assertions)`）
  - `bash -n scripts/l18/gui_real_cert.sh`：PASS（`wave106_gui_static_syntax_check.txt`）

### 🆕 L21 wave#106 推进快照（2026-03-06 03:51）

- 状态：（wave#106 已完成 router_rules_port_range 测试样例中的 fallback 从  调整为 ，并将区间外默认断言更新为 ，去除示例中的 silent direct fallback 字面量 + V7 断言升级）
- 本轮落地：
  1. ：router_rules_port_range 测试样例中的 fallback 从  调整为 ，并将区间外默认断言更新为 ，去除示例中的 silent direct fallback 字面量。
  2.  升级到 （272 assertions），新增 W106-01~W106-02。
  3. 回流阻断证据：（将  注入回  后  失败，）。
- 最小验证：
  - ：PASS（）
  - ：PASS（）
...
### 🆕 L21 wave#105 推进快照（2026-03-06 03:52)

- 状态：`MIG-02 hardening`（wave#105 已完成 router_select_ctx_meta 测试样例 default 去 silent direct fallback + V7 断言升级）
- 本轮落地：
  1. `crates/sb-core/tests/router_select_ctx_meta.rs`：router_select_ctx_meta 测试样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，并将 final 分支断言更新为 `unresolved`，去除示例中的 silent direct fallback 字面量。
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.102-wave105-v1`（270 assertions），新增 W105-01~W105-02。
  3. 回流阻断证据：`reports/l21/artifacts/wave105_v7_regression_block.txt`（将 `default=unresolved` 注入回 `default=direct` 后 `--v7-only` 失败，`exit_code=1`）。
- 最小验证：
  - `cargo check -p app --tests`：PASS（`wave105_wp1_app_tests_check.txt`）
  - `cargo check -p sb-core`：PASS（`wave105_wp1_sb_core_check.txt`）
  - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`V7 PASS (270 assertions)`）
  - `bash -n scripts/l18/gui_real_cert.sh`：PASS（`wave105_gui_static_syntax_check.txt`）
...
- 本轮落地：
  1. ：router_override 测试样例中的基础默认从  调整为 ，去除示例中的 silent direct fallback 字面量。
  2.  升级到 （268 assertions），新增 W104-01~W104-02。
  3. 回流阻断证据：（将  注入回  后  失败，）。
- 最小验证：
  - ：PASS（）
  - ：PASS（）
  - === 依赖边界检查 (2026-03-06 03:50) ===
...
### 🆕 L21 wave#103 推进快照（2026-03-06 03:42）

- 状态：`MIG-02 hardening`（wave#103 已完成 router_summary 测试样例 default 去 silent direct fallback + V7 断言升级）
- 本轮落地：
  1. `crates/sb-core/tests/router_summary.rs`：测试样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，去除示例中的 silent direct fallback 字面量。
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.100-wave103-v1`（266 assertions），新增 W103-01~W103-02。
  3. 回流阻断证据：`reports/l21/artifacts/wave103_v7_regression_block.txt`（将 `default=unresolved` 注入回 `default=direct` 后 `--v7-only` 失败，`exit_code=1`）。
- 最小验证：
  - `cargo check -p app --tests`：PASS（`wave103_wp1_app_tests_check.txt`）
  - `cargo check -p sb-core`：PASS（`wave103_wp1_sb_core_check.txt`）
  - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`V7 PASS (266 assertions)`）
  - `bash -n scripts/l18/gui_real_cert.sh`：PASS（`wave103_gui_static_syntax_check.txt`）

### 🆕 L21 wave#102 推进快照（2026-03-06 03:35）

- 状态：`MIG-02 hardening`（wave#102 已完成 router_rules_file 测试样例 default 去 silent direct fallback + V7 断言升级）
- 本轮落地：
  1. `crates/sb-core/tests/router_rules_file.rs`：测试样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，去除示例中的 silent direct fallback 字面量。
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.99-wave102-v1`（264 assertions），新增 W102-01~W102-02。
  3. 回流阻断证据：`reports/l21/artifacts/wave102_v7_regression_block.txt`（将 `default=unresolved` 注入回 `default=direct` 后 `--v7-only` 失败，`exit_code=1`）。
- 最小验证：
  - `cargo check -p app --tests`：PASS（`wave102_wp1_app_tests_check.txt`）
  - `cargo check -p sb-core`：PASS（`wave102_wp1_sb_core_check.txt`）
  - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`V7 PASS (264 assertions)`）
  - `bash -n scripts/l18/gui_real_cert.sh`：PASS（`wave102_gui_static_syntax_check.txt`）

### 🆕 L21 wave#101 推进快照（2026-03-06 03:33）

- 状态：`MIG-02 hardening`（wave#101 已完成 router_rules_portset 测试样例 default 去 silent direct fallback + V7 断言升级）
- 本轮落地：
  1. `crates/sb-core/tests/router_rules_portset.rs`：测试样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，并将默认命中断言更新为 `unresolved`，去除示例中的 silent direct fallback 字面量。
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.98-wave101-v1`（262 assertions），新增 W101-01~W101-02。
  3. 回流阻断证据：`reports/l21/artifacts/wave101_v7_regression_block.txt`（将 `default=unresolved` 注入回 `default=direct` 后 `--v7-only` 失败，`exit_code=1`）。
- 最小验证：
  - `cargo check -p app --tests`：PASS（`wave101_wp1_app_tests_check.txt`）
  - `cargo check -p sb-core`：PASS（`wave101_wp1_sb_core_check.txt`）
  - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`V7 PASS (262 assertions)`）
  - `bash -n scripts/l18/gui_real_cert.sh`：PASS（`wave101_gui_static_syntax_check.txt`）

### 🆕 L21 wave#100 推进快照（2026-03-06 03:30）

- 状态：`MIG-02 hardening`（wave#100 已完成 router_cache_transport 测试样例 default 去 silent direct fallback + V7 断言升级）
- 本轮落地：
  1. `crates/sb-core/tests/router_cache_transport.rs`：测试样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，去除示例中的 silent direct fallback 字面量。
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.97-wave100-v1`（260 assertions），新增 W100-01~W100-02。
  3. 回流阻断证据：`reports/l21/artifacts/wave100_v7_regression_block.txt`（将 `default=unresolved` 注入回 `default=direct` 后 `--v7-only` 失败，`exit_code=1`）。
- 最小验证：
  - `cargo check -p app --tests`：PASS（`wave100_wp1_app_tests_check.txt`）
  - `cargo check -p sb-core`：PASS（`wave100_wp1_sb_core_check.txt`）
  - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`V7 PASS (260 assertions)`）
  - `bash -n scripts/l18/gui_real_cert.sh`：PASS（`wave100_gui_static_syntax_check.txt`）

### 🆕 L21 wave#99 推进快照（2026-03-06 03:21）

- 状态：`MIG-02 hardening`（wave#99 已完成 router_rules_reload_noop 测试样例 default 去 silent direct fallback + V7 断言升级）
- 本轮落地：
  1. `crates/sb-core/tests/router_rules_reload_noop.rs`：测试样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`（两处），去除示例中的 silent direct fallback 字面量。
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.96-wave99-v1`（258 assertions），新增 W99-01~W99-02。
  3. 回流阻断证据：`reports/l21/artifacts/wave99_v7_regression_block.txt`（将 `default=unresolved` 注入回 `default=direct` 后 `--v7-only` 失败，`exit_code=1`）。
- 最小验证：
  - `cargo check -p app --tests`：PASS（`wave99_wp1_app_tests_check.txt`）
  - `cargo check -p sb-core`：PASS（`wave99_wp1_sb_core_check.txt`）
  - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`V7 PASS (258 assertions)`）
  - `bash -n scripts/l18/gui_real_cert.sh`：PASS（`wave99_gui_static_syntax_check.txt`）

### 🆕 L21 wave#98 推进快照（2026-03-06 03:17）
- 状态：`MIG-02 hardening`（wave#98 已完成 router_rules_index 测试样例 default 去 silent direct fallback + V7 断言升级）
- 本轮落地：
  1. `crates/sb-core/tests/router_rules_index.rs`：多处测试样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，并将无匹配默认断言更新为 `unresolved`，去除示例中的 silent direct fallback 字面量。
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.95-wave98-v1`（256 assertions），新增 W98-01~W98-02。
  3. 回流阻断证据：`reports/l21/artifacts/wave98_v7_regression_block.txt`（将 `default=unresolved` 注入回 `default=direct` 后 `--v7-only` 失败，`exit_code=1`）。
- 最小验证：
  - `cargo check -p app --tests`：PASS（`wave98_wp1_app_tests_check.txt`）
  - `cargo check -p sb-core`：PASS（`wave98_wp1_sb_core_check.txt`）
  - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`V7 PASS (256 assertions)`）
  - `bash -n scripts/l18/gui_real_cert.sh`：PASS（`wave98_gui_static_syntax_check.txt`）

### 🆕 L21 wave#97 推进快照（2026-03-06 03:14）
- 状态：`MIG-02 hardening`（wave#97 已完成 router_rules_include 测试样例 default 去 silent direct fallback + V7 断言升级）
- 本轮落地：
  1. `crates/sb-core/tests/router_rules_include.rs`：测试样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，去除示例中的 silent direct fallback 字面量。
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.94-wave97-v1`（254 assertions），新增 W97-01~W97-02。
  3. 回流阻断证据：`reports/l21/artifacts/wave97_v7_regression_block.txt`（将 `default=unresolved` 注入回 `default=direct` 后 `--v7-only` 失败，`exit_code=1`）。
- 最小验证：
  - `cargo check -p app --tests`：PASS（`wave97_wp1_app_tests_check.txt`）
  - `cargo check -p sb-core`：PASS（`wave97_wp1_sb_core_check.txt`）
  - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`V7 PASS (254 assertions)`）
  - `bash -n scripts/l18/gui_real_cert.sh`：PASS（`wave97_gui_static_syntax_check.txt`）

### 🆕 L21 wave#96 推进快照（2026-03-06 03:10）
- 状态：`MIG-02 hardening`（wave#96 已完成 router_resolver_async 测试样例 default 去 silent direct fallback + V7 断言升级）
- 本轮落地：
  1. `crates/sb-core/tests/router_resolver_async.rs`：测试样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，并将 timeout/error 默认回退断言更新为 `unresolved`，去除示例中的 silent direct fallback 字面量。
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.93-wave96-v1`（252 assertions），新增 W96-01~W96-02。
  3. 回流阻断证据：`reports/l21/artifacts/wave96_v7_regression_block.txt`（将 `default=unresolved` 注入回 `default=direct` 后 `--v7-only` 失败，`exit_code=1`）。
- 最小验证：
  - `cargo check -p app --tests`：PASS（`wave96_wp1_app_tests_check.txt`）
  - `cargo check -p sb-core`：PASS（`wave96_wp1_sb_core_check.txt`）
  - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`V7 PASS (252 assertions)`）
  - `bash -n scripts/l18/gui_real_cert.sh`：PASS（`wave96_gui_static_syntax_check.txt`）

### 🆕 L21 wave#95 推进快照（2026-03-06 02:16）
- 状态：`MIG-02 hardening`（wave#95 已完成 router_suffix_strict 测试样例 default 去 silent direct fallback + V7 断言升级）
- 本轮落地：
  1. `crates/sb-core/tests/router_suffix_strict.rs`：测试样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，并将对应断言更新为 `unresolved`，去除示例中的 silent direct fallback 字面量。
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.92-wave95-v1`（250 assertions），新增 W95-01~W95-02。
  3. 回流阻断证据：`reports/l21/artifacts/wave95_v7_regression_block.txt`（将 `default=unresolved` 注入回 `default=direct` 后 `--v7-only` 失败，`exit_code=1`）。
- 最小验证：
  - `cargo check -p app --tests`：PASS（`wave95_wp1_app_tests_check.txt`）
  - `cargo check -p sb-core`：PASS（`wave95_wp1_sb_core_check.txt`）
  - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`V7 PASS (250 assertions)`）
  - `bash -n scripts/l18/gui_real_cert.sh`：PASS（`wave95_gui_static_syntax_check.txt`）
...
- 本轮落地：
  1. `crates/sb-core/examples/rule_hot_reload_demo.rs`：hot reload 示例规则中的 fallback 从 `default=direct` 调整为 `default=unresolved`（两处），去除示例中的 silent direct fallback 字面量。
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.90-wave93-v1`（246 assertions），新增 W93-01~W93-02。
  3. 回流阻断证据：`reports/l21/artifacts/wave93_v7_regression_block.txt`（将 `default=unresolved` 注入回 `default=direct` 后 `--v7-only` 失败，`exit_code=1`）。
- 最小验证：
  - `cargo check -p app --tests`：PASS（`wave93_wp1_app_tests_check.txt`）
  - `cargo check -p sb-core`：PASS（`wave93_wp1_sb_core_check.txt`）
  - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`V7 PASS (246 assertions)`）
  - `bash -n scripts/l18/gui_real_cert.sh`：PASS（`wave93_gui_static_syntax_check.txt`）

### 🆕 L21 wave#92 推进快照（2026-03-06 02:06）

- 状态：`MIG-02 hardening`（wave#92 已完成 process routing demo default 去 silent direct fallback + V7 断言升级）
- 本轮落地：
  1. `crates/sb-core/examples/process_routing_demo.rs`：process routing 示例规则中的 fallback 从 `default=direct` 调整为 `default=unresolved`，去除示例中的 silent direct fallback 字面量。
  2. `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.89-wave92-v1`（244 assertions），新增 W92-01~W92-02。
  3. 回流阻断证据：`reports/l21/artifacts/wave92_v7_regression_block.txt`（将 `default=unresolved` 注入回 `default=direct` 后 `--v7-only` 失败，`exit_code=1`）。
- 最小验证：
  - `cargo check -p app --tests`：PASS（`wave92_wp1_app_tests_check.txt`）
  - `cargo check -p sb-core`：PASS（`wave92_wp1_sb_core_check.txt`）
  - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`V7 PASS (244 assertions)`）
  - `bash -n scripts/l18/gui_real_cert.sh`：PASS（`wave92_gui_static_syntax_check.txt`）
