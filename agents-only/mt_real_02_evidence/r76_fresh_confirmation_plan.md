...
- entry_gate: Confirmation gate: use the local `/tmp` mapping to generate the cohort-only neutralized subset, then re-run `reality_vless_sample_intake.py`. Because fresh02/fresh06 already entered the post-R73 rollup, expect covered_existing=2, fresh_ready=0, duplicate=0, not_ready=0. Require explicit user authorization listing this cohort by name and BHV 52/56 unchanged at gate time.
...
- **D**: Confirmation gate fails (covered_existing != 2, fresh_ready != 0, duplicate != 0, not_ready != 0, BHV != 52/56, or no explicit authorization). Live MUST NOT start.
...
- entry_gate: Confirmation gate: use the local `/tmp` mapping to generate the cohort-only neutralized subset, then re-run `reality_vless_sample_intake.py`. Because fresh03/fresh04/fresh05/fresh07 already entered the post-R73 rollup, expect covered_existing=4, fresh_ready=0, duplicate=0, not_ready=0. Authorization must explicitly list this cohort. BHV 52/56 unchanged.
...
- **D**: Confirmation gate fails (covered_existing != 4, fresh_ready != 0, duplicate != 0, not_ready != 0, BHV != 52/56, or no explicit authorization). Live MUST NOT start.
...
- entry_gate: Confirmation gate: use the local `/tmp` mapping to generate the 3-rep neutralized subset, then re-run `reality_vless_sample_intake.py`. Because fresh01/fresh09/fresh15 already entered the post-R73 rollup, expect covered_existing=3, fresh_ready=0, duplicate=0, not_ready=0. Authorization must list these 3 outbounds explicitly. BHV 52/56 unchanged. Reps were chosen by ordinal spread (fresh01 lower, fresh09 middle, fresh15 upper) to avoid clustering on one geographic region; ordinal spread is the only signal available without re-leaking original tags.
...
- **C**: Tooling/dataplane blocker; abort.
- **D**: Confirmation gate fails (covered_existing != 3, fresh_ready != 0, duplicate != 0, not_ready != 0, BHV != 52/56, or no explicit authorization). Live MUST NOT start.
...
- **confirmation-gate intake** — Re-derives the cohort subset classification after using the local `/tmp` mapping to generate a neutralized subset. For post-R73 confirmation cohorts, expect covered_existing=selected_count and fresh_ready=0.
  ```
...
- **confirmation plan dry-run** — Confirms target/timeout/runs/selected_count without opening a socket. Mirrors the no-live dry-run shape used before R73 execution.
  ```
