...
- **L18 Phase 2（Post-MIG-02 开封首跑）已规划**：6 Batch（E-J）/ 18 WP，目标是在 MIG-02 后代码基线上首次端到端跑通 L18 全链路。
  - 详见 `agents-only/03-planning/15-L18-PHASE2-POST-MIG02-WORKPACKAGES.md`
- 后续可转向 MIG-03 (Hysteria2) / MIG-04 (HTTP/Mixed) / MIG-05 (Transport) 的具体迁移，或其他 codebase hardening。
...
**状态**：✅ 完成一段（router `SB_ROUTER_RULES_INCLUDE_DEPTH` parse-failure 不再 silently collapse）；✅ strict gate allowlist 升级到 `l21.157-wave160-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-core/src/router/mod.rs`：新增 `parse_router_rules_include_depth_env(...)` 与 `router_rules_include_depth_from_env()`，include 递归深度 env 不再把 invalid `SB_ROUTER_RULES_INCLUDE_DEPTH` 静默折叠成默认值，改为显式告警后回退到默认 `4`
   - 新增最小单元测试，锁定 `SB_ROUTER_RULES_INCLUDE_DEPTH` helper 的显式报错口径
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.157-wave160-v1`，断言扩展到 `380` 条。
   - `reports/l21/artifacts/wave160_v7_regression_block.txt`：在临时 root 将 `router_rules_include_depth_from_env()` 注回旧 `v.parse::<usize>().ok()` 后，`--v7-only` 预期失败，`exit_code=1`。
3. 验证：
   - `wave160_wp1_app_tests_check.txt` PASS
   - `wave160_wp1_sb_core_check.txt` PASS
   - `wave160_sb_core_router_tests_check.txt` PASS（定向 `sb-core` 单测编译）
   - `wave160_strict_gate.txt` PASS
   - `wave160_v7_regression_block.txt` PASS（负样例按预期 FAIL，`exit_code=1`）
   - `wave160_gui_static_syntax_check.txt` PASS
4. 当前盘点：
   - 当前新增收口点：router `SB_ROUTER_RULES_INCLUDE_DEPTH` parse-failure 不再 silently collapse。

## 🆕 最新进展：L21 wave#159 推进落地（2026-03-06 19:49）

**状态**：✅ 完成一段（router `SB_ROUTER_RULES_MAX_DEPTH` parse-failure 不再 silently collapse）；✅ strict gate allowlist 升级到 `l21.156-wave159-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-core/src/router/mod.rs`：新增 `parse_router_rules_max_depth_env(...)` 与 `router_rules_max_depth_from_env()`，rules prepass 深度 env 不再把 invalid `SB_ROUTER_RULES_MAX_DEPTH` 静默折叠成默认值，改为显式告警后回退到默认 `3`
   - 新增最小单元测试，锁定 `SB_ROUTER_RULES_MAX_DEPTH` helper 的显式报错口径
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.156-wave159-v1`，断言扩展到 `378` 条。
   - `reports/l21/artifacts/wave159_v7_regression_block.txt`：在临时 root 将 `router_rules_max_depth_from_env()` 注回旧 `v.parse::<usize>().ok()` 后，`--v7-only` 预期失败，`exit_code=1`。
3. 验证：
   - `wave159_wp1_app_tests_check.txt` PASS
   - `wave159_wp1_sb_core_check.txt` PASS
   - `wave159_sb_core_router_tests_check.txt` PASS（定向 `sb-core` 单测编译）
   - `wave159_strict_gate.txt` PASS
   - `wave159_v7_regression_block.txt` PASS（负样例按预期 FAIL，`exit_code=1`）
   - `wave159_gui_static_syntax_check.txt` PASS
4. 当前盘点：
   - 当前新增收口点：router `SB_ROUTER_RULES_MAX_DEPTH` parse-failure 不再 silently collapse。

## 🆕 最新进展：L21 wave#158 推进落地（2026-03-06 19:42）

**状态**：✅ 完成一段（router `SB_ROUTER_RULES_BACKOFF_MAX_MS` parse-failure 不再 silently collapse）；✅ strict gate allowlist 升级到 `l21.155-wave158-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-core/src/router/mod.rs`：新增 `parse_router_rules_backoff_max_ms_env(...)` 与 `router_rules_backoff_max_ms_from_env()`，热重载 backoff cap env 不再把 invalid `SB_ROUTER_RULES_BACKOFF_MAX_MS` 静默折叠成默认值，改为显式告警后回退到默认 `30000`
   - 新增最小单元测试，锁定 `SB_ROUTER_RULES_BACKOFF_MAX_MS` helper 的显式报错口径
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.155-wave158-v1`，断言扩展到 `376` 条。
   - `reports/l21/artifacts/wave158_v7_regression_block.txt`：在临时 root 将 `router_rules_backoff_max_ms_from_env()` 注回旧 `v.parse::<u64>().ok()` 后，`--v7-only` 预期失败，`exit_code=1`。
3. 验证：
   - `wave158_wp1_app_tests_check.txt` PASS
   - `wave158_wp1_sb_core_check.txt` PASS
   - `wave158_sb_core_router_tests_check.txt` PASS（定向 `sb-core` 单测编译）
   - `wave158_strict_gate.txt` PASS
   - `wave158_v7_regression_block.txt` PASS（负样例按预期 FAIL，`exit_code=1`）
   - `wave158_gui_static_syntax_check.txt` PASS
4. 当前盘点：
   - 当前新增收口点：router `SB_ROUTER_RULES_BACKOFF_MAX_MS` parse-failure 不再 silently collapse。

## 🆕 最新进展：L21 wave#157 推进落地（2026-03-06 19:35）

**状态**：✅ 完成一段（router `SB_ROUTER_RULES_JITTER_MS` parse-failure 不再 silently collapse）；✅ strict gate allowlist 升级到 `l21.154-wave157-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-core/src/router/mod.rs`：新增 `parse_router_rules_jitter_ms_env(...)` 与 `router_rules_jitter_ms_from_env()`，热重载 jitter env 不再把 invalid `SB_ROUTER_RULES_JITTER_MS` 静默折叠成默认值，改为显式告警后回退到默认 `0`
   - 新增最小单元测试，锁定 `SB_ROUTER_RULES_JITTER_MS` helper 的显式报错口径
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.154-wave157-v1`，断言扩展到 `374` 条。
   - `reports/l21/artifacts/wave157_v7_regression_block.txt`：在临时 root 将 `router_rules_jitter_ms_from_env()` 注回旧 `v.parse::<u64>().ok()` 后，`--v7-only` 预期失败，`exit_code=1`。
3. 验证：
   - `wave157_wp1_app_tests_check.txt` PASS
   - `wave157_wp1_sb_core_check.txt` PASS
   - `wave157_sb_core_router_tests_check.txt` PASS（定向 `sb-core` 单测编译）
   - `wave157_strict_gate.txt` PASS
   - `wave157_v7_regression_block.txt` PASS（负样例按预期 FAIL，`exit_code=1`）
   - `wave157_gui_static_syntax_check.txt` PASS
4. 当前盘点：
   - 当前新增收口点：router `SB_ROUTER_RULES_JITTER_MS` parse-failure 不再 silently collapse。

## 🆕 最新进展：L21 wave#156 推进落地（2026-03-06 19:28）

**状态**：✅ 完成一段（router `SB_ROUTER_RULES_HOT_RELOAD_MS` parse-failure 不再 silently collapse）；✅ strict gate allowlist 升级到 `l21.153-wave156-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-core/src/router/mod.rs`：新增 `parse_router_rules_hot_reload_ms_env(...)` 与 `router_rules_hot_reload_ms_from_env()`，热重载间隔 env 不再把 invalid `SB_ROUTER_RULES_HOT_RELOAD_MS` 静默折叠成默认值，改为显式告警后回退到默认 `0`
   - 新增最小单元测试，锁定 `SB_ROUTER_RULES_HOT_RELOAD_MS` helper 的显式报错口径
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.153-wave156-v1`，断言扩展到 `372` 条。
   - `reports/l21/artifacts/wave156_v7_regression_block.txt`：在临时 root 将 `router_rules_hot_reload_ms_from_env()` 注回旧 `v.parse().ok()` 后，`--v7-only` 预期失败，`exit_code=1`。
3. 验证：
   - `wave156_wp1_app_tests_check.txt` PASS
   - `wave156_wp1_sb_core_check.txt` PASS
   - `wave156_sb_core_router_tests_check.txt` PASS（定向 `sb-core` 单测编译）
   - `wave156_strict_gate.txt` PASS
   - `wave156_v7_regression_block.txt` PASS（负样例按预期 FAIL，`exit_code=1`）
   - `wave156_gui_static_syntax_check.txt` PASS
4. 当前盘点：
   - 当前新增收口点：router `SB_ROUTER_RULES_HOT_RELOAD_MS` parse-failure 不再 silently collapse。

## 🆕 最新进展：L21 wave#155 推进落地（2026-03-06 19:22）

**状态**：✅ 完成一段（router `SB_ROUTER_RULES_MAX` parse-failure 不再 silently collapse）；✅ strict gate allowlist 升级到 `l21.152-wave155-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-core/src/router/mod.rs`：新增 `parse_router_rules_max_env(...)` 与 `router_rules_max_from_env()`，热重载 / env 初始化 / shared index 路径不再把 invalid `SB_ROUTER_RULES_MAX` 静默折叠成默认值，改为显式告警后回退到默认 `8192`
   - 新增最小单元测试，锁定 `SB_ROUTER_RULES_MAX` helper 的显式报错口径
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.152-wave155-v1`，断言扩展到 `370` 条。
   - `reports/l21/artifacts/wave155_v7_regression_block.txt`：在临时 root 将 `router_rules_max_from_env()` 注回旧 `v.parse().ok()` 后，`--v7-only` 预期失败，`exit_code=1`。
3. 验证：
   - `wave155_wp1_app_tests_check.txt` PASS
   - `wave155_wp1_sb_core_check.txt` PASS
   - `wave155_sb_core_router_tests_check.txt` PASS（定向 `sb-core` 单测编译）
   - `wave155_strict_gate.txt` PASS
   - `wave155_v7_regression_block.txt` PASS（负样例按预期 FAIL，`exit_code=1`）
   - `wave155_gui_static_syntax_check.txt` PASS
4. 当前盘点：
   - 当前新增收口点：router `SB_ROUTER_RULES_MAX` parse-failure 不再 silently collapse。

## 🆕 最新进展：L21 wave#154 推进落地（2026-03-06 19:14）

**状态**：✅ 完成一段（adapter register `parse_listen_addr(...)` 不再 silently normalize/fallback）；✅ strict gate allowlist 升级到 `l21.151-wave154-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-adapters/src/register.rs`：`parse_listen_addr(...)` 不再通过 `parse().ok().or_else(...)` 静默规范化/静默失败；raw host 归一化到 `host:port` 时改为显式告警，非法地址改为显式告警并返回 `None`
   - 新增最小单元测试，锁定裸 `listen` host 的显式规范化路径和非法 host 的拒绝路径
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.151-wave154-v1`，断言扩展到 `368` 条。
   - `reports/l21/artifacts/wave154_v7_regression_block.txt`：在临时 root 将 helper 注回旧 `parse().ok().or_else(...)` 后，`--v7-only` 预期失败，`exit_code=1`。
3. 验证：
   - `wave154_wp1_app_tests_check.txt` PASS
   - `wave154_wp1_sb_core_check.txt` PASS
   - `wave154_sb_adapters_register_tests_check.txt` PASS（定向 `sb-adapters` 单测编译）
   - `wave154_strict_gate.txt` PASS
   - `wave154_v7_regression_block.txt` PASS（负样例按预期 FAIL，`exit_code=1`）
   - `wave154_gui_static_syntax_check.txt` PASS
4. 当前盘点：
   - 当前新增收口点：adapter register `parse_listen_addr(...)` 不再 silently normalize/fallback。

## 🆕 最新进展：L21 wave#153 推进落地（2026-03-06 19:06）

**状态**：✅ 完成一段（adapter register `vless` outbound server socket parse 不再 silently collapse）；✅ strict gate allowlist 升级到 `l21.150-wave153-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-adapters/src/register.rs`：`build_vless_outbound(...)` 切到 `parse_required_outbound_socket_addr(...)`，不再把 invalid `server:port` 静默折叠成 builder `None`，改为显式 invalid-config connector
   - 新增最小单元测试，锁定 vless server helper 的显式报错口径
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.150-wave153-v1`，断言扩展到 `366` 条。
   - `reports/l21/artifacts/wave153_v7_regression_block.txt`：在临时 root 将 vless server 解析注回旧 `parse::<SocketAddr>().ok()?` 后，`--v7-only` 预期失败，`exit_code=1`。
3. 验证：
   - `wave153_wp1_app_tests_check.txt` PASS
   - `wave153_wp1_sb_core_check.txt` PASS
   - `wave153_sb_adapters_register_tests_check.txt` PASS（定向 `sb-adapters` 单测编译）
   - `wave153_strict_gate.txt` PASS
   - `wave153_v7_regression_block.txt` PASS（负样例按预期 FAIL，`exit_code=1`）
   - `wave153_gui_static_syntax_check.txt` PASS
4. 当前盘点：
   - 当前新增收口点：adapter register 的 `vless` outbound server socket parse 不再 silently collapse。

## 🆕 最新进展：L21 wave#152 推进落地（2026-03-06 18:58）

**状态**：✅ 完成一段（adapter register `vmess` outbound server socket parse 不再 silently collapse）；✅ strict gate allowlist 升级到 `l21.149-wave152-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-adapters/src/register.rs`：新增 `parse_required_outbound_socket_addr(...)`，`build_vmess_outbound(...)` 不再把 invalid `server:port` 静默折叠成 builder `None`，改为显式 invalid-config connector
   - 新增最小单元测试，锁定 vmess server helper 的显式报错口径
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.149-wave152-v1`，断言扩展到 `364` 条。
   - `reports/l21/artifacts/wave152_v7_regression_block.txt`：在临时 root 将 vmess server 解析注回旧 `parse::<SocketAddr>().ok()?` 后，`--v7-only` 预期失败，`exit_code=1`。
3. 验证：
   - `wave152_wp1_app_tests_check.txt` PASS
   - `wave152_wp1_sb_core_check.txt` PASS
   - `wave152_sb_adapters_register_tests_check.txt` PASS（定向 `sb-adapters` 单测编译）
   - `wave152_strict_gate.txt` PASS
   - `wave152_v7_regression_block.txt` PASS（负样例按预期 FAIL，`exit_code=1`）
   - `wave152_gui_static_syntax_check.txt` PASS
4. 当前盘点：
   - 当前新增收口点：adapter register 的 `vmess` outbound server socket parse 不再 silently collapse。

## 🆕 最新进展：L21 wave#151 推进落地（2026-03-06 18:49）

**状态**：✅ 完成一段（geoip `SB_GEOIP_TTL` parse-failure 不再 silently collapse）；✅ strict gate allowlist 升级到 `l21.148-wave151-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-core/src/router/engine.rs`：新增 `parse_geoip_ttl_env(...)`，`init_geoip_if_env()` 不再把 invalid `SB_GEOIP_TTL` 静默折叠成默认值，改为显式告警后回退到默认 `600s`
   - 新增最小单元测试，锁定 geoip ttl env helper 的显式报错口径
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.148-wave151-v1`，断言扩展到 `362` 条。
   - `reports/l21/artifacts/wave151_v7_regression_block.txt`：在临时 root 将 geoip ttl 解析注回旧 `humantime::parse_duration(...).ok()` 后，`--v7-only` 预期失败，`exit_code=1`。
3. 验证：
   - `wave151_wp1_app_tests_check.txt` PASS
   - `wave151_wp1_sb_core_check.txt` PASS
   - `wave151_sb_core_router_tests_check.txt` PASS（定向 `sb-core` 单测编译）
   - `wave151_strict_gate.txt` PASS
   - `wave151_v7_regression_block.txt` PASS（负样例按预期 FAIL，`exit_code=1`）
   - `wave151_gui_static_syntax_check.txt` PASS
4. 当前盘点：
   - 当前新增收口点：geoip `SB_GEOIP_TTL` parse-failure 不再 silently collapse。

## 🆕 最新进展：L21 wave#150 推进落地（2026-03-06 18:41）

**状态**：✅ 完成一段（geoip `SB_GEOIP_CACHE` parse-failure 不再 silently collapse）；✅ strict gate allowlist 升级到 `l21.147-wave150-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-core/src/router/engine.rs`：新增 `parse_geoip_cache_cap_env(...)`，`init_geoip_if_env()` 不再把 invalid `SB_GEOIP_CACHE` 静默折叠成默认值，改为显式告警后回退到默认 `8192`
   - 新增最小单元测试，锁定 geoip cache env helper 的显式报错口径
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.147-wave150-v1`，断言扩展到 `360` 条。
   - `reports/l21/artifacts/wave150_v7_regression_block.txt`：在临时 root 将 geoip cache 解析注回旧 `v.parse().ok()` 后，`--v7-only` 预期失败，`exit_code=1`。
3. 验证：
   - `wave150_wp1_app_tests_check.txt` PASS
   - `wave150_wp1_sb_core_check.txt` PASS
   - `wave150_sb_core_router_tests_check.txt` PASS（定向 `sb-core` 单测编译）
   - `wave150_strict_gate.txt` PASS
   - `wave150_v7_regression_block.txt` PASS（负样例按预期 FAIL，`exit_code=1`）
   - `wave150_gui_static_syntax_check.txt` PASS
4. 当前盘点：
   - 当前新增收口点：geoip `SB_GEOIP_CACHE` parse-failure 不再 silently collapse。

## 🆕 最新进展：L21 wave#149 推进落地（2026-03-06 18:33）

**状态**：✅ 完成一段（router `SB_ROUTER_UDP_RULES` parse-failure 不再 silently collapse）；✅ strict gate allowlist 升级到 `l21.146-wave149-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-core/src/router/engine.rs`：新增 `parse_udp_rules_index(...)`，`udp_rules_index_from_env()` 不再把 invalid `SB_ROUTER_UDP_RULES` 静默折叠成 `None`，改为显式告警后拒绝加载该 env rules 索引
   - 新增最小单元测试，锁定 udp env rules helper 的显式报错口径
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.146-wave149-v1`，断言扩展到 `358` 条。
   - `reports/l21/artifacts/wave149_v7_regression_block.txt`：在临时 root 将 udp env rules 解析注回旧 `.ok()?` 后，`--v7-only` 预期失败，`exit_code=1`。
3. 验证：
   - `wave149_wp1_app_tests_check.txt` PASS
   - `wave149_wp1_sb_core_check.txt` PASS
   - `wave149_sb_core_router_tests_check.txt` PASS（定向 `sb-core` 单测编译）
   - `wave149_strict_gate.txt` PASS
   - `wave149_v7_regression_block.txt` PASS（负样例按预期 FAIL，`exit_code=1`）
   - `wave149_gui_static_syntax_check.txt` PASS
4. 当前盘点：
   - 当前新增收口点：router `SB_ROUTER_UDP_RULES` parse-failure 不再 silently collapse。
...
**状态**：✅ 完成一段（adapter register `shadowsocks` outbound invalid connector config 不再 silently collapse）；✅ strict gate allowlist 升级到 `l21.144-wave147-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-adapters/src/register.rs`：新增 `invalid_outbound_config_reason(...)`，`build_shadowsocks_outbound(...)` 不再把 `ShadowsocksConnector::new(cfg)` 失败静默折叠成 builder `None`，改为显式 invalid-config connector
   - 新增最小单元测试，锁定 shadowsocks invalid-config helper 的显式报错口径
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.144-wave147-v1`，断言扩展到 `354` 条。
   - `reports/l21/artifacts/wave147_v7_regression_block.txt`：在临时 root 将 shadowsocks builder 注回旧 `.ok()?` 后，`--v7-only` 预期失败，`exit_code=1`。
3. 验证：
   - `wave147_wp1_app_tests_check.txt` PASS
   - `wave147_wp1_sb_core_check.txt` PASS
   - `wave147_sb_adapters_register_tests_check.txt` PASS（定向 `sb-adapters` 单测编译）
   - `wave147_strict_gate.txt` PASS
   - `wave147_v7_regression_block.txt` PASS（负样例按预期 FAIL，`exit_code=1`）
   - `wave147_gui_static_syntax_check.txt` PASS
4. 当前盘点：
   - 当前新增收口点：adapter register 的 shadowsocks outbound invalid connector config 不再 silently collapse。

## 🆕 最新进展：L21 wave#146 推进落地（2026-03-06 18:07）

**状态**：✅ 完成一段（adapter register `dns` outbound `server` IP 配置解析不再 silently collapse）；✅ strict gate allowlist 升级到 `l21.143-wave146-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-adapters/src/register.rs`：新增 `parse_required_outbound_ip_addr(...)`，`build_dns_outbound(...)` 不再把 invalid `server` IP 静默折叠成 builder `None`，改为显式 invalid-config connector；缺失 `server` 仍保持原有 `None`
   - 新增最小单元测试，锁定 dns server helper 的显式报错口径
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.143-wave146-v1`，断言扩展到 `352` 条。
   - `reports/l21/artifacts/wave146_v7_regression_block.txt`：在临时 root 将 dns server 解析注回旧 `parse().ok()` 后，`--v7-only` 预期失败，`exit_code=1`。
3. 验证：
   - `wave146_wp1_app_tests_check.txt` PASS
   - `wave146_wp1_sb_core_check.txt` PASS
   - `wave146_sb_adapters_register_tests_check.txt` PASS（定向 `sb-adapters` 单测编译）
   - `wave146_strict_gate.txt` PASS
   - `wave146_v7_regression_block.txt` PASS（负样例按预期 FAIL，`exit_code=1`）
   - `wave146_gui_static_syntax_check.txt` PASS
4. 当前盘点：
   - 当前新增收口点：adapter register 的 dns outbound `server` IP 解析不再 silently collapse。

## 🆕 最新进展：L21 wave#145 推进落地（2026-03-06 17:56）

**状态**：✅ 完成一段（adapter register `tuic` outbound `uuid` 配置解析不再 silently collapse）；✅ strict gate allowlist 升级到 `l21.142-wave145-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-adapters/src/register.rs`：`build_tuic_outbound(...)` 切到 `parse_required_outbound_uuid(...)`，不再把 invalid `uuid` 静默折叠成 builder `None`，改为显式 invalid-config connector；缺失 `uuid` 仍保持 `None`
   - 新增最小单元测试，锁定 tuic uuid helper 的显式报错口径
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.142-wave145-v1`，断言扩展到 `350` 条。
   - `reports/l21/artifacts/wave145_v7_regression_block.txt`：在临时 root 将 tuic uuid 解析注回旧 `parse_str(...).ok()?` 后，`--v7-only` 预期失败，`exit_code=1`。
3. 验证：
   - `wave145_wp1_app_tests_check.txt` PASS
   - `wave145_wp1_sb_core_check.txt` PASS
   - `wave145_sb_adapters_register_tests_check.txt` PASS（定向 `sb-adapters` 单测编译）
   - `wave145_strict_gate.txt` PASS
   - `wave145_v7_regression_block.txt` PASS（负样例按预期 FAIL，`exit_code=1`）
   - `wave145_gui_static_syntax_check.txt` PASS
4. 当前盘点：
   - 当前新增收口点：adapter register 的 tuic outbound uuid 解析不再 silently collapse。

## 🆕 最新进展：L21 wave#144 推进落地（2026-03-06 17:53）

**状态**：✅ 完成一段（adapter register `vless` outbound `uuid` 配置解析不再 silently collapse）；✅ strict gate allowlist 升级到 `l21.141-wave144-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-adapters/src/register.rs`：`build_vless_outbound(...)` 切到 `parse_required_outbound_uuid(...)`，不再把 invalid `uuid` 静默折叠成 builder `None`，改为显式 invalid-config connector；缺失 `uuid` 仍保持 `None`
   - 新增最小单元测试，锁定 vless uuid helper 的显式报错口径，并把 helper 测试移到无条件 `#[cfg(test)]` 模块，确保默认特性下也会编译
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.141-wave144-v1`，断言扩展到 `348` 条。
   - `reports/l21/artifacts/wave144_v7_regression_block.txt`：在临时 root 将 vless uuid 解析注回旧 `parse_str(...).ok()?` 后，`--v7-only` 预期失败，`exit_code=1`。
3. 验证：
   - `wave144_wp1_app_tests_check.txt` PASS
   - `wave144_wp1_sb_core_check.txt` PASS
   - `wave144_sb_adapters_register_tests_check.txt` PASS（定向 `sb-adapters` 单测编译）
   - `wave144_strict_gate.txt` PASS
   - `wave144_v7_regression_block.txt` PASS（负样例按预期 FAIL，`exit_code=1`）
   - `wave144_gui_static_syntax_check.txt` PASS
4. 当前盘点：
   - 当前新增收口点：adapter register 的 vless outbound uuid 解析不再 silently collapse。

## 🆕 最新进展：L21 wave#143 推进落地（2026-03-06 17:47）

**状态**：✅ 完成一段（adapter register `vmess` outbound `uuid` 配置解析不再 silently collapse）；✅ strict gate allowlist 升级到 `l21.140-wave143-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-adapters/src/register.rs`：新增 `parse_required_outbound_uuid(...)` 与 `InvalidConfigConnector`，`build_vmess_outbound(...)` 不再把 invalid `uuid` 静默折叠成 builder `None`，改为显式 invalid-config connector；缺失 `uuid` 仍保持 `None`
   - 新增最小单元测试，锁定 vmess uuid helper 的显式报错口径
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.140-wave143-v1`，断言扩展到 `346` 条。
   - `reports/l21/artifacts/wave143_v7_regression_block.txt`：在临时 root 将 vmess uuid 解析注回旧 `parse_str(...).ok()?` 后，`--v7-only` 预期失败，`exit_code=1`。
3. 验证：
   - `wave143_wp1_app_tests_check.txt` PASS
   - `wave143_wp1_sb_core_check.txt` PASS
   - `wave143_sb_adapters_register_tests_check.txt` PASS（定向 `sb-adapters` 单测编译）
   - `wave143_strict_gate.txt` PASS
   - `wave143_v7_regression_block.txt` PASS（负样例按预期 FAIL，`exit_code=1`）
   - `wave143_gui_static_syntax_check.txt` PASS
4. 当前盘点：
   - 当前新增收口点：adapter register 的 vmess outbound uuid 解析不再 silently collapse。

## 🆕 最新进展：L21 wave#142 推进落地（2026-03-06 17:41）

**状态**：✅ 完成一段（adapter bridge outbound `inet4_bind_address` / `inet6_bind_address` 配置解析不再 silently ignore）；✅ strict gate allowlist 升级到 `l21.139-wave142-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-core/src/adapter/bridge.rs`：新增 `parse_optional_outbound_ipv4_addr(...)` 与 `parse_optional_outbound_ipv6_addr(...)`，`to_outbound_param(...)` 不再把 invalid bind address 静默吞成 `None`，改为显式报错；outbound 组装路径沿用上一波的显式拒绝逻辑
   - 新增最小单元测试，锁定 helper 与 `to_outbound_param(...)` 的 invalid bind address 显式报错口径
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.139-wave142-v1`，断言扩展到 `344` 条。
   - `reports/l21/artifacts/wave142_v7_regression_block.txt`：在临时 root 将 bind address 解析注回旧 `parse().ok()` 后，`--v7-only` 预期失败，`exit_code=1`。
3. 验证：
   - `wave142_wp1_app_tests_check.txt` PASS
   - `wave142_wp1_sb_core_check.txt` PASS
   - `wave142_sb_core_bridge_tests_check.txt` PASS（定向 `sb-core` 单测编译）
   - `wave142_strict_gate.txt` PASS
   - `wave142_v7_regression_block.txt` PASS（负样例按预期 FAIL，`exit_code=1`）
   - `wave142_gui_static_syntax_check.txt` PASS
4. 当前盘点：
   - 当前新增收口点：adapter bridge 出站参数转换里的 bind address 解析不再 silently ignore。

## 🆕 最新进展：L21 wave#141 推进落地（2026-03-06 17:38）

**状态**：✅ 完成一段（adapter bridge outbound `connect_timeout` 配置解析不再 silently ignore）；✅ strict gate allowlist 升级到 `l21.138-wave141-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-core/src/adapter/bridge.rs`：新增 `parse_optional_outbound_duration(...)`，`to_outbound_param(...)` 不再把 invalid `connect_timeout` 静默吞成 `None`，改为显式报错；`assemble_outbounds(...)` / `assemble_selectors(...)` 统一拒绝构建该 outbound adapter 并打出迁移提示
   - 新增最小单元测试，锁定 helper 与 `to_outbound_param(...)` 的 invalid duration 显式报错口径
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.138-wave141-v1`，断言扩展到 `342` 条。
   - `reports/l21/artifacts/wave141_v7_regression_block.txt`：在临时 root 将 outbound `connect_timeout` 解析注回旧 `parse_duration(...).ok()` 后，`--v7-only` 预期失败，`exit_code=1`。
3. 验证：
   - `wave141_wp1_app_tests_check.txt` PASS
   - `wave141_wp1_sb_core_check.txt` PASS
   - `wave141_sb_core_bridge_tests_check.txt` PASS（定向 `sb-core` 单测编译）
   - `wave141_strict_gate.txt` PASS
   - `wave141_v7_regression_block.txt` PASS（负样例按预期 FAIL，`exit_code=1`）
   - `wave141_gui_static_syntax_check.txt` PASS
4. 当前盘点：
   - 当前新增收口点：adapter bridge 出站参数转换里的 `connect_timeout` 解析不再 silently ignore。

## 🆕 最新进展：L21 wave#140 推进落地（2026-03-06 17:29）

**状态**：✅ 完成一段（adapter bridge inbound `udp_timeout` 配置解析不再 silently ignore）；✅ strict gate allowlist 升级到 `l21.137-wave140-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-core/src/adapter/bridge.rs`：新增 `parse_optional_inbound_duration(...)`，`to_inbound_param(...)` 不再把 invalid `udp_timeout` 静默吞成 `None`，改为显式报错；`build_bridge(...)` / `build_bridge(no-router)` 统一拒绝构建该 inbound adapter 并打出迁移提示
   - 新增最小单元测试，锁定 helper 与 `to_inbound_param(...)` 的 invalid duration 显式报错口径
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.137-wave140-v1`，断言扩展到 `340` 条。
   - `reports/l21/artifacts/wave140_v7_regression_block.txt`：在临时 root 移除 bridge helper 调用后，`--v7-only` 预期失败，`exit_code=1`。
3. 验证：
   - `wave140_wp1_app_tests_check.txt` PASS
   - `wave140_wp1_sb_core_check.txt` PASS
   - `wave140_sb_core_bridge_tests_check.txt` PASS（定向 `sb-core` 单测编译）
   - `wave140_strict_gate.txt` PASS
   - `wave140_v7_regression_block.txt` PASS（负样例按预期 FAIL，`exit_code=1`）
   - `wave140_gui_static_syntax_check.txt` PASS
4. 当前盘点：
   - 当前新增收口点：adapter bridge 入站参数转换里的 `udp_timeout` 解析不再 silently ignore。

## 🆕 最新进展：L21 wave#139 推进落地（2026-03-06 17:23）

**状态**：✅ 完成一段（vmess inbound `uuid` 配置解析不再 silently collapse）；✅ strict gate allowlist 升级到 `l21.136-wave139-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `app/src/inbound_starter.rs`：`start_vmess_inbound(...)` 切到 `parse_optional_inbound_uuid(...)`，不再把 invalid `uuid` 静默折叠进 “missing or invalid” skip，改为显式报错并拒绝启动 inbound；缺失 `uuid` 仍保持 skip
   - 新增最小单元测试，锁定 vmess uuid helper 必须带上协议标签
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.136-wave139-v1`，断言扩展到 `338` 条。
   - `reports/l21/artifacts/wave139_v7_regression_block.txt`：在临时 root 将 vmess uuid 解析注回旧 `parse_str(...).ok()` 后，`--v7-only` 预期失败，`exit_code=1`。
3. 验证：
   - `wave139_wp1_app_tests_check.txt` PASS
   - `wave139_wp1_sb_core_check.txt` PASS
   - `wave139_inbound_starter_tests_check.txt` PASS（定向 `app` 单测编译）
   - `wave139_strict_gate.txt` PASS
   - `wave139_v7_regression_block.txt` PASS（负样例按预期 FAIL，`exit_code=1`）
   - `wave139_gui_static_syntax_check.txt` PASS
4. 当前盘点：
   - 当前新增收口点：vmess inbound 启动配置里的 invalid `uuid` 解析不再 silently collapse。

## 🆕 最新进展：L21 wave#138 推进落地（2026-03-06 17:20）

**状态**：✅ 完成一段（vless inbound `uuid` 配置解析不再 silently collapse）；✅ strict gate allowlist 升级到 `l21.135-wave138-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `app/src/inbound_starter.rs`：新增 `parse_optional_inbound_uuid(...)`，`start_vless_inbound(...)` 不再把 invalid `uuid` 静默折叠进 “missing or invalid” skip，改为显式报错并拒绝启动 inbound；缺失 `uuid` 仍保持 skip
   - 新增最小单元测试，锁定 vless uuid helper 必须带上协议标签
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.135-wave138-v1`，断言扩展到 `336` 条。
   - `reports/l21/artifacts/wave138_v7_regression_block.txt`：在临时 root 将 vless uuid 解析注回旧 `parse_str(...).ok()` 后，`--v7-only` 预期失败，`exit_code=1`。
3. 验证：
   - `wave138_wp1_app_tests_check.txt` PASS
   - `wave138_wp1_sb_core_check.txt` PASS
   - `wave138_inbound_starter_tests_check.txt` PASS（定向 `app` 单测编译）
   - `wave138_strict_gate.txt` PASS
   - `wave138_v7_regression_block.txt` PASS（负样例按预期 FAIL，`exit_code=1`）
   - `wave138_gui_static_syntax_check.txt` PASS
4. 当前盘点：
   - 当前新增收口点：vless inbound 启动配置里的 invalid `uuid` 解析不再 silently collapse。
...
**状态**：✅ 完成一段（socks inbound `udp_timeout` 配置解析不再 silently ignore）；✅ strict gate allowlist 升级到 `l21.133-wave136-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `app/src/inbound_starter.rs`：新增 `parse_optional_inbound_duration(...)`，`start_socks_inbound(...)` 不再把无效 `udp_timeout` 静默吞成 `None`，改为显式报错并拒绝启动 inbound
   - 新增最小单元测试，锁定无效 duration 必须显式报错
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.133-wave136-v1`，断言扩展到 `332` 条。
   - `reports/l21/artifacts/wave136_v7_regression_block.txt`：在临时 root 将 socks `udp_timeout` 解析注回旧 `parse_duration(...).ok()` 后，`--v7-only` 预期失败，`exit_code=1`。
3. 验证：
   - `wave136_wp1_app_tests_check.txt` PASS
   - `wave136_wp1_sb_core_check.txt` PASS
   - `wave136_inbound_starter_tests_check.txt` PASS（定向 `app` 单测编译）
   - `wave136_strict_gate.txt` PASS
   - `wave136_v7_regression_block.txt` PASS（负样例按预期 FAIL，`exit_code=1`）
   - `wave136_gui_static_syntax_check.txt` PASS
4. 当前盘点：
   - 当前新增收口点：socks inbound 启动配置里的 `udp_timeout` 解析不再 silently ignore。

## 🆕 最新进展：L21 wave#135 推进落地（2026-03-06 17:06）

**状态**：✅ 完成一段（tun inbound unsupported decision 不再 silently fallback 到 direct）；✅ strict gate allowlist 升级到 `l21.132-wave135-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-adapters/src/inbound/tun/mod.rs`：新增 `route_target_from_decision(...)`，使 tun inbound runtime connect path 与辅助 TCP packet path 不再把 `Sniff/Resolve/Hijack/HijackDns/Proxy(None)` 等 unsupported 决策静默回落到 `direct`，改为显式 `Unsupported` 错误并给迁移提示
   - 新增最小单元测试，锁定 unsupported decision 显式报错与 named proxy tag 保留
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.132-wave135-v1`，断言扩展到 `330` 条。
   - `reports/l21/artifacts/wave135_v7_regression_block.txt`：在临时 root 篡改 helper call 后，`--v7-only` 预期失败，`exit_code=1`。
3. 验证：
   - `wave135_wp1_app_tests_check.txt` PASS
   - `wave135_wp1_sb_core_check.txt` PASS
   - `wave135_tun_inbound_tests_check.txt` PASS（定向 `sb-adapters` 单测编译）
   - `wave135_strict_gate.txt` PASS
   - `wave135_v7_regression_block.txt` PASS（负样例按预期 FAIL，`exit_code=1`）
   - `wave135_gui_static_syntax_check.txt` PASS
4. 当前盘点：
   - 当前新增收口点：tun inbound runtime 路由决策不再 silently fallback 到 `direct`。

## 🆕 最新进展：L21 wave#134 推进落地（2026-03-06 16:53）

**状态**：✅ 完成一段（vless inbound fallback 配置解析不再 silently ignore）；✅ strict gate allowlist 升级到 `l21.131-wave134-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `app/src/inbound_starter.rs`：`start_vless_inbound(...)` 切到 `parse_optional_inbound_fallback_addr(...)` 与 `parse_inbound_fallback_for_alpn(...)`，不再把无效 `fallback` / `fallback_for_alpn` 静默吞成 `None`，改为显式报错并拒绝启动 inbound
   - 新增最小单元测试，锁定 vless fallback helper 必须带上协议标签
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.131-wave134-v1`，断言扩展到 `328` 条。
   - `reports/l21/artifacts/wave134_v7_regression_block.txt`：在临时 root 将 vless fallback 解析注回旧 `parse(...).ok()` 后，`--v7-only` 预期失败，`exit_code=1`。
3. 验证：
   - `wave134_wp1_app_tests_check.txt` PASS
   - `wave134_wp1_sb_core_check.txt` PASS
   - `wave134_inbound_starter_tests_check.txt` PASS（定向 `app` 单测编译）
   - `wave134_strict_gate.txt` PASS
   - `wave134_v7_regression_block.txt` PASS（负样例按预期 FAIL，`exit_code=1`）
   - `wave134_gui_static_syntax_check.txt` PASS
4. 当前盘点：
   - 当前新增收口点：vless inbound 启动配置里的 fallback 地址解析不再 silently ignore。

## 🆕 最新进展：L21 wave#133 推进落地（2026-03-06 16:50）

**状态**：✅ 完成一段（vmess inbound fallback 配置解析不再 silently ignore）；✅ strict gate allowlist 升级到 `l21.130-wave133-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `app/src/inbound_starter.rs`：`start_vmess_inbound(...)` 切到 `parse_optional_inbound_fallback_addr(...)` 与 `parse_inbound_fallback_for_alpn(...)`，不再把无效 `fallback` / `fallback_for_alpn` 静默吞成 `None`，改为显式报错并拒绝启动 inbound
   - 新增最小单元测试，锁定 vmess fallback helper 必须带上协议标签
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.130-wave133-v1`，断言扩展到 `326` 条。
   - `reports/l21/artifacts/wave133_v7_regression_block.txt`：在临时 root 将 vmess fallback 解析注回旧 `parse(...).ok()` 后，`--v7-only` 预期失败，`exit_code=1`。
3. 验证：
   - `wave133_wp1_app_tests_check.txt` PASS
   - `wave133_wp1_sb_core_check.txt` PASS
   - `wave133_inbound_starter_tests_check.txt` PASS（定向 `app` 单测编译）
   - `wave133_strict_gate.txt` PASS
   - `wave133_v7_regression_block.txt` PASS（负样例按预期 FAIL，`exit_code=1`）
   - `wave133_gui_static_syntax_check.txt` PASS
4. 当前盘点：
   - 当前新增收口点：vmess inbound 启动配置里的 fallback 地址解析不再 silently ignore。
...
**状态**：✅ 完成一段（tuic inbound route-target kind 不再 silently 标记为 direct）；✅ strict gate allowlist 升级到 `l21.128-wave131-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-adapters/src/inbound/tuic.rs`：新增 `decision_from_route_target(...)`，使 `OutboundKind::Socks/Http/Naive/Hysteria2` 等 kind 路由目标不再落成 `Decision::Direct`，统一改为显式 `Decision::Proxy(Some(<kind>))`
   - 新增最小单元测试，覆盖 proxy kind、direct/block、以及 named proxy 三类 route target 映射
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.128-wave131-v1`，断言扩展到 `322` 条。
   - `reports/l21/artifacts/wave131_v7_regression_block.txt`：在临时 root 将 helper call 注回旧 `_ => Decision::Direct` 后，`--v7-only` 预期失败，`exit_code=1`。
3. 验证：
   - `wave131_wp1_app_tests_check.txt` PASS
   - `wave131_wp1_sb_core_check.txt` PASS
   - `wave131_tuic_route_target_tests_check.txt` PASS（定向 `adapter-tuic` 单测）
   - `wave131_strict_gate.txt` PASS
   - `wave131_v7_regression_block.txt` PASS（负样例按预期 FAIL，`exit_code=1`）
   - `wave131_gui_static_syntax_check.txt` PASS
4. 当前盘点：
   - 当前新增收口点：tuic inbound 连接元数据不再 silently fallback 到 `direct`。
...
**状态**：✅ 完成一段（naive inbound route-target kind 不再 silently 标记为 direct）；✅ strict gate allowlist 升级到 `l21.126-wave129-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-adapters/src/inbound/naive.rs`：新增 `decision_from_route_target(...)`，使 `OutboundKind::Socks/Http/Naive/Hysteria2` 等 kind 路由目标不再落成 `Decision::Direct`，统一改为显式 `Decision::Proxy(Some(<kind>))`
   - 新增最小单元测试，覆盖 proxy kind、direct/block、以及 named proxy 三类 route target 映射
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.126-wave129-v1`，断言扩展到 `318` 条。
   - `reports/l21/artifacts/wave129_v7_regression_block.txt`：在临时 root 将 helper call 注回旧 `_ => Decision::Direct` 后，`--v7-only` 预期失败，`exit_code=1`。
3. 验证：
   - `wave129_wp1_app_tests_check.txt` PASS
   - `wave129_wp1_sb_core_check.txt` PASS
   - `wave129_naive_route_target_tests_check.txt` PASS（定向 `adapter-naive` 单测）
   - `wave129_strict_gate.txt` PASS
   - `wave129_v7_regression_block.txt` PASS（负样例按预期 FAIL，`exit_code=1`）
   - `wave129_gui_static_syntax_check.txt` PASS
4. 当前盘点：
   - 当前新增收口点：naive inbound 连接元数据不再 silently fallback 到 `direct`。
...
**状态**：✅ 完成一段（shadowsocks inbound unsupported decision 不再 direct fallback）；✅ strict gate allowlist 升级到 `l21.123-wave126-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-adapters/src/inbound/shadowsocks.rs`：将 `Hijack/Sniff/Resolve/HijackDns` 等 unsupported 决策从 `_ => direct` 收口为显式错误，禁止静默走 direct
   - `crates/sb-adapters/src/inbound/shadowsocks.rs` 内测试：锁定旧 `direct for now` 注释不得回流，并要求保留新的显式错误口径
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.123-wave126-v1`，断言扩展到 `312` 条。
   - `reports/l21/artifacts/wave126_v7_regression_block.txt`：在临时 root 恢复旧注释/直连回退后，`--v7-only` 预期失败，`exit_code=1`。
3. 验证：
...
4. 当前盘点：
   - 当前新增收口点：Shadowsocks 入站 unsupported 路由决策不再 silently fallback 到 direct。
...
**状态**：✅ 完成一段（router_hot_reload_integration 测试样例 default 去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.119-wave122-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-core/tests/router_hot_reload_integration.rs`：将 hot reload 集成样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，去除示例中的 silent direct fallback 字面量
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.119-wave122-v1`，断言扩展到 `304` 条。
   - `reports/l21/artifacts/wave122_v7_regression_block.txt`：注入回流样例后 `--v7-only` 失败，`exit_code=1`。
3. 验证：
...
## 🆕 最新进展：L21 wave#121 推进落地（2026-03-06 04:41）

**状态**：✅ 完成一段（router_geosite_rules_integration 测试样例 default 去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.118-wave121-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-core/tests/router_geosite_rules_integration.rs`：将 GeoSite 集成样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，并将对应默认值断言同步更新为 `unresolved`
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.118-wave121-v1`，断言扩展到 `302` 条。
   - `reports/l21/artifacts/wave121_v7_regression_block.txt`：注入回流样例后 `--v7-only` 失败，`exit_code=1`。
3. 验证：
...
4. 剩余规模：
   - `crates/sb-core/tests` 尚余 `1` 个测试文件、`10` 处 `default=direct`。
...
**状态**：✅ 完成一段（router_cidr4 测试样例 default 去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.116-wave119-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-core/tests/router_cidr4.rs`：将 CIDR4 规则样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，并将未命中/非法规则断言同步更新为 `unresolved`
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.116-wave119-v1`，断言扩展到 `298` 条。
   - `reports/l21/artifacts/wave119_v7_regression_block.txt`：注入回流样例后 `--v7-only` 失败，`exit_code=1`。
3. 验证：
...
4. 剩余规模：
   - `crates/sb-core/tests` 尚余 `3` 个测试文件、`17` 处 `default=direct`。

## 🆕 最新进展：L21 wave#118 推进落地（2026-03-06 04:21）

**状态**：✅ 完成一段（router_hot_reload_integration_complete 测试样例 default 去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.115-wave118-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-core/tests/router_hot_reload_integration_complete.rs`：将 hot reload 完整集成样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，去除示例中的 silent direct fallback 字面量
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.115-wave118-v1`，断言扩展到 `296` 条。
   - `reports/l21/artifacts/wave118_v7_regression_block.txt`：注入回流样例后 `--v7-only` 失败，`exit_code=1`。
3. 验证：
...
4. 剩余规模：
   - `crates/sb-core/tests` 尚余 `4` 个测试文件、`20` 处 `default=direct`。

## 🆕 最新进展：L21 wave#117 推进落地（2026-03-06 04:20）

**状态**：✅ 完成一段（router_ipversion_matching 测试样例 default 去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.114-wave117-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-core/tests/router_ipversion_matching.rs`：将 ipversion 解析样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，去除示例中的 silent direct fallback 字面量
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.114-wave117-v1`，断言扩展到 `294` 条。
   - `reports/l21/artifacts/wave117_v7_regression_block.txt`：注入回流样例后 `--v7-only` 失败，`exit_code=1`。
3. 验证：
...
4. 剩余规模：
   - `crates/sb-core/tests` 尚余 `5` 个测试文件、`22` 处 `default=direct`。

## 🆕 最新进展：L21 wave#116 推进落地（2026-03-06 04:20）

**状态**：✅ 完成一段（router_query_type_matching 测试样例 default 去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.113-wave116-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-core/tests/router_query_type_matching.rs`：将 query_type 解析样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，去除示例中的 silent direct fallback 字面量
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.113-wave116-v1`，断言扩展到 `292` 条。
   - `reports/l21/artifacts/wave116_v7_regression_block.txt`：注入回流样例后 `--v7-only` 失败，`exit_code=1`。
3. 验证：
...
4. 剩余规模：
   - `crates/sb-core/tests` 尚余 `6` 个测试文件、`24` 处 `default=direct`。

## 🆕 最新进展：L21 wave#115 推进落地（2026-03-06 04:19）

**状态**：✅ 完成一段（router_rules_port_transport 测试样例 default 去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.112-wave115-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-core/tests/router_rules_port_transport.rs`：将 port/transport 规则样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，去除示例中的 silent direct fallback 字面量
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.112-wave115-v1`，断言扩展到 `290` 条。
   - `reports/l21/artifacts/wave115_v7_regression_block.txt`：注入回流样例后 `--v7-only` 失败，`exit_code=1`。
3. 验证：
...
4. 剩余规模：
   - `crates/sb-core/tests` 尚余 `7` 个测试文件、`26` 处 `default=direct`。

## 🆕 最新进展：L21 wave#114 推进落地（2026-03-06 04:18）

**状态**：✅ 完成一段（router_udp_rules 测试样例 default 去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.111-wave114-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-core/tests/router_udp_rules.rs`：将环境规则样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，并将未命中断言同步更新为 `unresolved`
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.111-wave114-v1`，断言扩展到 `288` 条。
   - `reports/l21/artifacts/wave114_v7_regression_block.txt`：注入回流样例后 `--v7-only` 失败，`exit_code=1`。
3. 验证：
...
4. 剩余规模：
   - `crates/sb-core/tests` 尚余 `8` 个测试文件、`29` 处 `default=direct`。

## 🆕 最新进展：L21 wave#113 推进落地（2026-03-06 04:18）

**状态**：✅ 完成一段（router_rules_include_cycle 测试样例 default 去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.110-wave113-v1`；✅ 回流阻断负样例证据更新

1. 本轮落地：
   - `crates/sb-core/tests/router_rules_include_cycle.rs`：将 include cycle 相关样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，去除示例中的 silent direct fallback 字面量
2. V7 升级：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.110-wave113-v1`，断言扩展到 `286` 条。
   - `reports/l21/artifacts/wave113_v7_regression_block.txt`：注入回流样例后 `--v7-only` 失败，`exit_code=1`。
3. 验证：
...
4. 剩余规模：
   - `crates/sb-core/tests` 尚余 `9` 个测试文件、`30` 处 `default=direct`。
...
**状态**：✅ `MIG-02 wave#109` 完成一段（router_domain_regex_matching 测试样例 default 去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.106-wave109-v1`；✅ 回流阻断负样例证据更新

1. **推进 wave#109（MIG-02 hardening，router_domain_regex_matching 测试样例 default 去 silent direct fallback 路径）**：
   - `crates/sb-core/tests/router_domain_regex_matching.rs`：
     - 将两处测试样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，去除示例中的 silent direct fallback 字面量。
2. **strict gate allowlist 升级（V7 wave#109）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.106-wave109-v1`，断言扩展到 278 条（新增 W109-01/W109-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave109_v7_regression_block.txt`（在临时 root 将 `default=unresolved` 注入回 `default=direct` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave109_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave109_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave109_strict_gate.txt`，`V7 PASS (278 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave109_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave109_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave109_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave109_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave109_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave109_gui_static_syntax_check.txt`）

---

## 🆕 最新进展：L21 wave#108 推进落地（2026-03-06 03:54)

**状态**：✅ `MIG-02 wave#108` 完成一段（router_cache 测试样例 default 去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.105-wave108-v1`；✅ 回流阻断负样例证据更新

1. **推进 wave#108（MIG-02 hardening，router_cache 测试样例 default 去 silent direct fallback 路径）**：
   - `crates/sb-core/tests/router_cache.rs`：
     - 将两处测试样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，去除示例中的 silent direct fallback 字面量。
2. **strict gate allowlist 升级（V7 wave#108）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.105-wave108-v1`，断言扩展到 276 条（新增 W108-01/W108-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave108_v7_regression_block.txt`（在临时 root 将 `default=unresolved` 注入回 `default=direct` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave108_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave108_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave108_strict_gate.txt`，`V7 PASS (276 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave108_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave108_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave108_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave108_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave108_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave108_gui_static_syntax_check.txt`）

---

## 🆕 最新进展：L21 wave#107 推进落地（2026-03-06 03:53）

**状态**：✅  完成一段（router_explain 测试样例 default 去 silent direct fallback）；✅ strict gate allowlist 升级到 ；✅ 回流阻断负样例证据更新

1. **推进 wave#107（MIG-02 hardening，router_explain 测试样例 default 去 silent direct fallback 路径）**：
   - ：
     - router_explain 测试样例 default 去 silent direct fallback。
2. **strict gate allowlist 升级（V7 wave#107）**：
   -  升级到 ，断言扩展到 274 条（新增 W107-01/W107-02）。
   - 回流阻断证据：（在临时 root 将  注入回  后， 预期失败，）。
3. **门禁与编译复验**：
   - ：PASS（）。
   - ：PASS（）。
   - === 依赖边界检查 (2026-03-06 03:53) ===
...
── V6: strict feature tree / default closure / reverse deps ──
  ...
  PASS
── V7: L20 migration assertions ──
  INFO: assertion version: l21.104-wave107-v1
  PASS (280 assertions)
...
**状态**：✅ `MIG-02 wave#107` 完成一段（router_explain 测试样例 default 去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.104-wave107-v1`；✅ 回流阻断负样例证据更新

1. **推进 wave#107（MIG-02 hardening，router_explain 测试样例 default 去 silent direct fallback 路径）**：
   - `crates/sb-core/tests/router_explain.rs`：
     - 将测试样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，去除示例中的 silent direct fallback 字面量。
2. **strict gate allowlist 升级（V7 wave#107）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.104-wave107-v1`，断言扩展到 274 条（新增 W107-01/W107-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave107_v7_regression_block.txt`（在临时 root 将 `default=unresolved` 注入回 `default=direct` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave107_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave107_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave107_strict_gate.txt`，`V7 PASS (274 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave107_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave107_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave107_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave107_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave107_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave107_gui_static_syntax_check.txt`）
...
**状态**：✅ `MIG-02 wave#105` 完成一段（router_select_ctx_meta 测试样例 default 去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.102-wave105-v1`；✅ 回流阻断负样例证据更新

1. **推进 wave#105（MIG-02 hardening，router_select_ctx_meta 测试样例 default 去 silent direct fallback 路径）**：
...
2. **strict gate allowlist 升级（V7 wave#105）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.102-wave105-v1`，断言扩展到 270 条（新增 W105-01/W105-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave105_v7_regression_block.txt`（在临时 root 将 `default=unresolved` 注入回 `default=direct` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave105_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave105_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave105_strict_gate.txt`，`V7 PASS (270 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave105_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave105_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave105_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave105_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave105_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave105_gui_static_syntax_check.txt`）

---

## 🆕 最新进展：L21 wave#106 推进落地（2026-03-06 03:51）

**状态**：✅  完成一段（router_rules_port_range 测试样例 default 去 silent direct fallback）；✅ strict gate allowlist 升级到 ；✅ 回流阻断负样例证据更新

1. **推进 wave#106（MIG-02 hardening，router_rules_port_range 测试样例 default 去 silent direct fallback 路径）**：
   - ：
     - router_rules_port_range 测试样例 default 去 silent direct fallback。
2. **strict gate allowlist 升级（V7 wave#106）**：
   -  升级到 ，断言扩展到 272 条（新增 W106-01/W106-02）。
   - 回流阻断证据：（在临时 root 将  注入回  后， 预期失败，）。
3. **门禁与编译复验**：
   - ：PASS（）。
   - ：PASS（）。
   - === 依赖边界检查 (2026-03-06 03:52) ===
...
## 🆕 最新进展：L21 wave#104 推进落地（2026-03-06 03:52)

**状态**：✅ `MIG-02 wave#104` 完成一段（router_override 测试样例基础默认去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.101-wave104-v1`；✅ 回流阻断负样例证据更新

1. **推进 wave#104（MIG-02 hardening，router_override 测试样例基础默认去 silent direct fallback 路径）**：
   - `crates/sb-core/tests/router_override.rs`：
...
2. **strict gate allowlist 升级（V7 wave#104）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.101-wave104-v1`，断言扩展到 268 条（新增 W104-01/W104-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave104_v7_regression_block.txt`（在临时 root 将 `default=unresolved` 注入回 `default=direct` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave104_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave104_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave104_strict_gate.txt`，`V7 PASS (268 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave104_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave104_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave104_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave104_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave104_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave104_gui_static_syntax_check.txt`）

---

## 🆕 最新进展：L21 wave#105 推进落地（2026-03-06 03:50）

**状态**：✅  完成一段（router_select_ctx_meta 测试样例 default 去 silent direct fallback）；✅ strict gate allowlist 升级到 ；✅ 回流阻断负样例证据更新

1. **推进 wave#105（MIG-02 hardening，router_select_ctx_meta 测试样例 default 去 silent direct fallback 路径）**：
   - ：
     - router_select_ctx_meta 测试样例 default 去 silent direct fallback。
2. **strict gate allowlist 升级（V7 wave#105）**：
   -  升级到 ，断言扩展到 270 条（新增 W105-01/W105-02）。
   - 回流阻断证据：（在临时 root 将  注入回  后， 预期失败，）。
3. **门禁与编译复验**：
   - ：PASS（）。
   - ：PASS（）。
...
## 🆕 最新进展：L21 wave#103 推进落地（2026-03-06 03:42）

**状态**：✅ `MIG-02 wave#103` 完成一段（router_summary 测试样例 default 去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.100-wave103-v1`；✅ 回流阻断负样例证据更新

1. **推进 wave#103（MIG-02 hardening，router_summary 测试样例路径）**：
   - `crates/sb-core/tests/router_summary.rs`：
     - 测试样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，去除示例中的 silent direct fallback 字面量。
2. **strict gate allowlist 升级（V7 wave#103）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.100-wave103-v1`，断言扩展到 266 条（新增 W103-01/W103-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave103_v7_regression_block.txt`（在临时 root 将 `default=unresolved` 注入回 `default=direct` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave103_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave103_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave103_strict_gate.txt`，`V7 PASS (266 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave103_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave103_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave103_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave103_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave103_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave103_gui_static_syntax_check.txt`）

---

## 🆕 最新进展：L21 wave#102 推进落地（2026-03-06 03:35）

**状态**：✅ `MIG-02 wave#102` 完成一段（router_rules_file 测试样例 default 去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.99-wave102-v1`；✅ 回流阻断负样例证据更新

1. **推进 wave#102（MIG-02 hardening，router_rules_file 测试样例路径）**：
   - `crates/sb-core/tests/router_rules_file.rs`：
     - 测试样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，去除示例中的 silent direct fallback 字面量。
2. **strict gate allowlist 升级（V7 wave#102）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.99-wave102-v1`，断言扩展到 264 条（新增 W102-01/W102-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave102_v7_regression_block.txt`（在临时 root 将 `default=unresolved` 注入回 `default=direct` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave102_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave102_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave102_strict_gate.txt`，`V7 PASS (264 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave102_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave102_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave102_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave102_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave102_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave102_gui_static_syntax_check.txt`）

---

## 🆕 最新进展：L21 wave#101 推进落地（2026-03-06 03:33）

**状态**：✅ `MIG-02 wave#101` 完成一段（router_rules_portset 测试样例 default 去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.98-wave101-v1`；✅ 回流阻断负样例证据更新

1. **推进 wave#101（MIG-02 hardening，router_rules_portset 测试样例路径）**：
   - `crates/sb-core/tests/router_rules_portset.rs`：
     - 测试样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，并将默认命中断言更新为 `unresolved`，去除示例中的 silent direct fallback 字面量。
2. **strict gate allowlist 升级（V7 wave#101）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.98-wave101-v1`，断言扩展到 262 条（新增 W101-01/W101-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave101_v7_regression_block.txt`（在临时 root 将 `default=unresolved` 注入回 `default=direct` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave101_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave101_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave101_strict_gate.txt`，`V7 PASS (262 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave101_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave101_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave101_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave101_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave101_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave101_gui_static_syntax_check.txt`）

---

## 🆕 最新进展：L21 wave#100 推进落地（2026-03-06 03:30）

**状态**：✅ `MIG-02 wave#100` 完成一段（router_cache_transport 测试样例 default 去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.97-wave100-v1`；✅ 回流阻断负样例证据更新

1. **推进 wave#100（MIG-02 hardening，router_cache_transport 测试样例路径）**：
   - `crates/sb-core/tests/router_cache_transport.rs`：
     - 测试样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，去除示例中的 silent direct fallback 字面量。
2. **strict gate allowlist 升级（V7 wave#100）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.97-wave100-v1`，断言扩展到 260 条（新增 W100-01/W100-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave100_v7_regression_block.txt`（在临时 root 将 `default=unresolved` 注入回 `default=direct` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave100_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave100_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave100_strict_gate.txt`，`V7 PASS (260 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave100_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave100_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave100_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave100_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave100_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave100_gui_static_syntax_check.txt`）
...
1. **推进 wave#99（MIG-02 hardening，router_rules_reload_noop 测试样例路径）**：
   - `crates/sb-core/tests/router_rules_reload_noop.rs`：
     - 测试样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`（两处），去除示例中的 silent direct fallback 字面量。
2. **strict gate allowlist 升级（V7 wave#99）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.96-wave99-v1`，断言扩展到 258 条（新增 W99-01/W99-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave99_v7_regression_block.txt`（在临时 root 将 `default=unresolved` 注入回 `default=direct` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave99_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave99_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave99_strict_gate.txt`，`V7 PASS (258 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave99_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave99_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave99_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave99_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave99_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave99_gui_static_syntax_check.txt`）

---

## 🆕 最新进展：L21 wave#98 推进落地（2026-03-06 03:17）

**状态**：✅ `MIG-02 wave#98` 完成一段（router_rules_index 测试样例 default 去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.95-wave98-v1`；✅ 回流阻断负样例证据更新

1. **推进 wave#98（MIG-02 hardening，router_rules_index 测试样例路径）**：
   - `crates/sb-core/tests/router_rules_index.rs`：
     - 多处测试样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，并将无匹配默认断言更新为 `unresolved`，去除示例中的 silent direct fallback 字面量。
2. **strict gate allowlist 升级（V7 wave#98）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.95-wave98-v1`，断言扩展到 256 条（新增 W98-01/W98-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave98_v7_regression_block.txt`（在临时 root 将 `default=unresolved` 注入回 `default=direct` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave98_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave98_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave98_strict_gate.txt`，`V7 PASS (256 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave98_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave98_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave98_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave98_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave98_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave98_gui_static_syntax_check.txt`）

---

## 🆕 最新进展：L21 wave#97 推进落地（2026-03-06 03:14）

**状态**：✅ `MIG-02 wave#97` 完成一段（router_rules_include 测试样例 default 去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.94-wave97-v1`；✅ 回流阻断负样例证据更新

1. **推进 wave#97（MIG-02 hardening，router_rules_include 测试样例路径）**：
   - `crates/sb-core/tests/router_rules_include.rs`：
     - 测试样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，去除示例中的 silent direct fallback 字面量。
2. **strict gate allowlist 升级（V7 wave#97）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.94-wave97-v1`，断言扩展到 254 条（新增 W97-01/W97-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave97_v7_regression_block.txt`（在临时 root 将 `default=unresolved` 注入回 `default=direct` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave97_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave97_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave97_strict_gate.txt`，`V7 PASS (254 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave97_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave97_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave97_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave97_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave97_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave97_gui_static_syntax_check.txt`）

---

## 🆕 最新进展：L21 wave#96 推进落地（2026-03-06 03:10）

**状态**：✅ `MIG-02 wave#96` 完成一段（router_resolver_async 测试样例 default 去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.93-wave96-v1`；✅ 回流阻断负样例证据更新

1. **推进 wave#96（MIG-02 hardening，router_resolver_async 测试样例路径）**：
   - `crates/sb-core/tests/router_resolver_async.rs`：
     - 测试样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，并将 timeout/error 默认回退断言更新为 `unresolved`，去除示例中的 silent direct fallback 字面量。
2. **strict gate allowlist 升级（V7 wave#96）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.93-wave96-v1`，断言扩展到 252 条（新增 W96-01/W96-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave96_v7_regression_block.txt`（在临时 root 将 `default=unresolved` 注入回 `default=direct` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave96_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave96_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave96_strict_gate.txt`，`V7 PASS (252 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave96_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave96_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave96_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave96_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave96_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave96_gui_static_syntax_check.txt`）

---

## 🆕 最新进展：L21 wave#95 推进落地（2026-03-06 02:16）

**状态**：✅ `MIG-02 wave#95` 完成一段（router_suffix_strict 测试样例 default 去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.92-wave95-v1`；✅ 回流阻断负样例证据更新

1. **推进 wave#95（MIG-02 hardening，router_suffix_strict 测试样例路径）**：
   - `crates/sb-core/tests/router_suffix_strict.rs`：
     - 测试样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，并将对应断言更新为 `unresolved`，去除示例中的 silent direct fallback 字面量。
2. **strict gate allowlist 升级（V7 wave#95）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.92-wave95-v1`，断言扩展到 250 条（新增 W95-01/W95-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave95_v7_regression_block.txt`（在临时 root 将 `default=unresolved` 注入回 `default=direct` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave95_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave95_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave95_strict_gate.txt`，`V7 PASS (250 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave95_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave95_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave95_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave95_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave95_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave95_gui_static_syntax_check.txt`）

---

## 🆕 最新进展：L21 wave#94 推进落地（2026-03-06 02:12）

**状态**：✅ `MIG-02 wave#94` 完成一段（router_rules_decide_with_meta 测试样例 default 去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.91-wave94-v1`；✅ 回流阻断负样例证据更新

1. **推进 wave#94（MIG-02 hardening，router_rules_decide_with_meta 测试样例路径）**：
   - `crates/sb-core/tests/router_rules_decide_with_meta.rs`：
     - 测试样例中的 fallback 从 `default=direct` 调整为 `default=unresolved`，去除示例中的 silent direct fallback 字面量。
2. **strict gate allowlist 升级（V7 wave#94）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.91-wave94-v1`，断言扩展到 248 条（新增 W94-01/W94-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave94_v7_regression_block.txt`（在临时 root 将 `default=unresolved` 注入回 `default=direct` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave94_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave94_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave94_strict_gate.txt`，`V7 PASS (248 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave94_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave94_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave94_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave94_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave94_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave94_gui_static_syntax_check.txt`）

---

## 🆕 最新进展：L21 wave#93 推进落地（2026-03-06 02:09）

**状态**：✅ `MIG-02 wave#93` 完成一段（rule hot reload demo default 去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.90-wave93-v1`；✅ 回流阻断负样例证据更新

1. **推进 wave#93（MIG-02 hardening，rule hot reload demo 路径）**：
   - `crates/sb-core/examples/rule_hot_reload_demo.rs`：
     - hot reload 示例规则中的 fallback 从 `default=direct` 调整为 `default=unresolved`（两处），去除示例中的 silent direct fallback 字面量。
2. **strict gate allowlist 升级（V7 wave#93）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.90-wave93-v1`，断言扩展到 246 条（新增 W93-01/W93-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave93_v7_regression_block.txt`（在临时 root 将 `default=unresolved` 注入回 `default=direct` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave93_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave93_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave93_strict_gate.txt`，`V7 PASS (246 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave93_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave93_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave93_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave93_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave93_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave93_gui_static_syntax_check.txt`）

---

## 🆕 最新进展：L21 wave#92 推进落地（2026-03-06 02:06）

**状态**：✅ `MIG-02 wave#92` 完成一段（process routing demo default 去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.89-wave92-v1`；✅ 回流阻断负样例证据更新

1. **推进 wave#92（MIG-02 hardening，process routing demo 路径）**：
   - `crates/sb-core/examples/process_routing_demo.rs`：
     - process routing 示例规则中的 fallback 从 `default=direct` 调整为 `default=unresolved`，去除示例中的 silent direct fallback 字面量。
2. **strict gate allowlist 升级（V7 wave#92）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.89-wave92-v1`，断言扩展到 244 条（新增 W92-01/W92-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave92_v7_regression_block.txt`（在临时 root 将 `default=unresolved` 注入回 `default=direct` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave92_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave92_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave92_strict_gate.txt`，`V7 PASS (244 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave92_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave92_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave92_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave92_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave92_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave92_gui_static_syntax_check.txt`）

---

## 🆕 最新进展：L21 wave#91 推进落地（2026-03-06 02:03）

**状态**：✅ `MIG-02 wave#91` 完成一段（geosite demo default 去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.88-wave91-v1`；✅ 回流阻断负样例证据更新

1. **推进 wave#91（MIG-02 hardening，geosite demo 路径）**：
   - `crates/sb-core/examples/geosite_demo.rs`：
     - GeoSite 示例规则中的 fallback 从 `default=direct` 调整为 `default=unresolved`，去除示例中的 silent direct fallback 字面量。
2. **strict gate allowlist 升级（V7 wave#91）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.88-wave91-v1`，断言扩展到 242 条（新增 W91-01/W91-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave91_v7_regression_block.txt`（在临时 root 将 `default=unresolved` 注入回 `default=direct` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave91_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave91_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave91_strict_gate.txt`，`V7 PASS (242 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave91_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave91_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave91_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave91_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave91_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave91_gui_static_syntax_check.txt`）

---

## 🆕 最新进展：L21 wave#90 推进落地（2026-03-06 01:58）

**状态**：✅ `MIG-02 wave#90` 完成一段（dsl sample default 去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.87-wave90-v1`；✅ 回流阻断负样例证据更新

1. **推进 wave#90（MIG-02 hardening，dsl sample 路径）**：
   - `app/examples/dsl.sample.txt`：
     - 默认规则从 `default=direct` 调整为 `default=unresolved`，去除示例中的 silent direct fallback 字面量。
2. **strict gate allowlist 升级（V7 wave#90）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.87-wave90-v1`，断言扩展到 240 条（新增 W90-01/W90-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave90_v7_regression_block.txt`（在临时 root 将 `default=unresolved` 注入回 `default=direct` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave90_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave90_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave90_strict_gate.txt`，`V7 PASS (240 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave90_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave90_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave90_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave90_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave90_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave90_gui_static_syntax_check.txt`）

---

## 🆕 最新进展：L21 wave#89 推进落地（2026-03-06 01:55）

**状态**：✅ `MIG-02 wave#89` 完成一段（router hot_reload validation 测试样例 default 去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.86-wave89-v1`；✅ 回流阻断负样例证据更新

1. **推进 wave#89（MIG-02 hardening，hot_reload 测试样例路径）**：
   - `crates/sb-core/src/router/hot_reload.rs`：
     - `test_rule_set_validation` 的 `valid_content` 从 `default=direct` 调整为 `default=unresolved`，去除测试样例中的 silent direct fallback 字面量。
2. **strict gate allowlist 升级（V7 wave#89）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.86-wave89-v1`，断言扩展到 238 条（新增 W89-01/W89-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave89_v7_regression_block.txt`（在临时 root 将 `default=unresolved` 注入回 `default=direct` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave89_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave89_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave89_strict_gate.txt`，`V7 PASS (238 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave89_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave89_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave89_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave89_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave89_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave89_gui_static_syntax_check.txt`）

---

## 🆕 最新进展：L21 wave#88 推进落地（2026-03-06 01:50）

**状态**：✅ `MIG-02 wave#88` 完成一段（validator v2 unknown outbound type fallback 去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.85-wave88-v1`；✅ 回流阻断负样例证据更新

1. **推进 wave#88（MIG-02 hardening，validator v2 unknown type 路径）**：
   - `crates/sb-config/src/validator/v2.rs`：
     - outbounds 类型解析中未知 `type` 分支从 `_ => OutboundType::Direct` 调整为 `_ => OutboundType::Block`，移除 silent direct fallback。
2. **strict gate allowlist 升级（V7 wave#88）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.85-wave88-v1`，断言扩展到 236 条（新增 W88-01/W88-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave88_v7_regression_block.txt`（在临时 root 将 `_ => OutboundType::Block` 注入回 `_ => OutboundType::Direct` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave88_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave88_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave88_strict_gate.txt`，`V7 PASS (236 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave88_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave88_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave88_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave88_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave88_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave88_gui_static_syntax_check.txt`）

---

## 🆕 最新进展：L21 wave#87 推进落地（2026-03-06 01:47）

**状态**：✅ `MIG-02 wave#87` 完成一段（validator v2 outbound type 默认决策去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.84-wave87-v1`；✅ 回流阻断负样例证据更新

1. **推进 wave#87（MIG-02 hardening，validator v2 路径）**：
   - `crates/sb-config/src/validator/v2.rs`：
     - outbounds 类型解析中缺失 `type` 时默认值从 `direct` 调整为 `unresolved`，去除 silent direct fallback 字面量。
2. **strict gate allowlist 升级（V7 wave#87）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.84-wave87-v1`，断言扩展到 234 条（新增 W87-01/W87-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave87_v7_regression_block.txt`（在临时 root 将 `unwrap_or(\"unresolved\")` 注入回 `unwrap_or(\"direct\")` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave87_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave87_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave87_strict_gate.txt`，`V7 PASS (234 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave87_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave87_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave87_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave87_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave87_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave87_gui_static_syntax_check.txt`）

---

## 🆕 最新进展：L21 wave#86 推进落地（2026-03-06 01:45）

**状态**：✅ `MIG-02 wave#86` 完成一段（bootstrap router rules text final default 默认决策去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.83-wave86-v1`；✅ 回流阻断负样例证据更新

1. **推进 wave#86（MIG-02 hardening，bootstrap router rules text final default 路径）**：
   - `app/src/bootstrap.rs`：
     - `ir_to_router_rules_text` 在缺失 `route.default` 时默认规则从 `default=direct` 调整为 `default=unresolved`，去除 silent direct fallback 字面量。
2. **strict gate allowlist 升级（V7 wave#86）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.83-wave86-v1`，断言扩展到 232 条（新增 W86-01/W86-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave86_v7_regression_block.txt`（在临时 root 将 `rules.push(\"default=unresolved\".to_string());` 注入回 `rules.push(\"default=direct\".to_string());` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave86_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave86_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave86_strict_gate.txt`，`V7 PASS (232 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave86_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave86_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave86_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave86_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave86_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave86_gui_static_syntax_check.txt`）

---

## 🆕 最新进展：L21 wave#85 推进落地（2026-03-06 01:42）

**状态**：✅ `MIG-02 wave#85` 完成一段（router mod default 默认决策去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.82-wave85-v1`；✅ 回流阻断负样例证据更新

1. **推进 wave#85（MIG-02 hardening，router mod 路径）**：
   - `crates/sb-core/src/router/mod.rs`：
     - 构建 `RouterIndex` 时 `default` 的默认决策从 `direct` 调整为 `unresolved`（两处），去除 silent direct fallback 字面量。
2. **strict gate allowlist 升级（V7 wave#85）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.82-wave85-v1`，断言扩展到 230 条（新增 W85-01/W85-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave85_v7_regression_block.txt`（在临时 root 将 `default: default.unwrap_or(\"unresolved\"),` 注入回 `default: default.unwrap_or(\"direct\"),` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave85_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave85_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave85_strict_gate.txt`，`V7 PASS (230 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave85_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave85_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave85_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave85_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave85_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave85_gui_static_syntax_check.txt`）

---

## 🆕 最新进展：L21 wave#84 推进落地（2026-03-06 01:39）

**状态**：✅ `MIG-02 wave#84` 完成一段（env_dump udp_proxy_mode 默认决策去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.81-wave84-v1`；✅ 回流阻断负样例证据更新

1. **推进 wave#84（MIG-02 hardening，env_dump 路径）**：
   - `app/src/env_dump.rs`：
     - `SB_UDP_PROXY_MODE` 缺失时默认值从 `direct` 调整为 `unresolved`，去除 silent direct fallback 字面量。
2. **strict gate allowlist 升级（V7 wave#84）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.81-wave84-v1`，断言扩展到 228 条（新增 W84-01/W84-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave84_v7_regression_block.txt`（在临时 root 将 `\"unresolved\".to_string()` 注入回 `\"direct\".to_string()` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave84_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave84_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave84_strict_gate.txt`，`V7 PASS (228 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave84_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave84_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave84_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave84_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave84_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave84_gui_static_syntax_check.txt`）

---

## 🆕 最新进展：L21 wave#83 推进落地（2026-03-06 01:36）

**状态**：✅ `MIG-02 wave#83` 完成一段（bootstrap router rules text rule_outbound 默认决策去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.80-wave83-v1`；✅ 回流阻断负样例证据更新

1. **推进 wave#83（MIG-02 hardening，bootstrap router rules text 路径）**：
   - `app/src/bootstrap.rs`：
     - `ir_to_router_rules_text` 中规则缺失 `outbound` 时默认决策从 `direct` 调整为 `unresolved`，去除 silent direct fallback 字面量。
2. **strict gate allowlist 升级（V7 wave#83）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.80-wave83-v1`，断言扩展到 226 条（新增 W83-01/W83-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave83_v7_regression_block.txt`（在临时 root 将 `unwrap_or(\"unresolved\")` 注入回 `unwrap_or(\"direct\")` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave83_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave83_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave83_strict_gate.txt`，`V7 PASS (226 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave83_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave83_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave83_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave83_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave83_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave83_gui_static_syntax_check.txt`）

---

## 🆕 最新进展：L21 wave#82 推进落地（2026-03-06 01:26）

**状态**：✅ `MIG-02 wave#82` 完成一段（adapter bridge final_rule 默认决策去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.79-wave82-v1`；✅ 回流阻断负样例证据更新

1. **推进 wave#82（MIG-02 hardening，adapter bridge final_rule 路径）**：
   - `crates/sb-core/src/adapter/bridge.rs`：
     - `final_rule` 在缺失 `route.default/final_outbound` 时默认决策从 `direct` 调整为 `unresolved`，去除 silent direct fallback 字面量。
2. **strict gate allowlist 升级（V7 wave#82）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.79-wave82-v1`，断言扩展到 224 条（新增 W82-01/W82-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave82_v7_regression_block.txt`（在临时 root 将 `unwrap_or(\"unresolved\")` 注入回 `unwrap_or(\"direct\")` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave82_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave82_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave82_strict_gate.txt`，`V7 PASS (224 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave82_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave82_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave82_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave82_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave82_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave82_gui_static_syntax_check.txt`）

---

## 🆕 最新进展：L21 wave#81 推进落地（2026-03-06 01:23）

**状态**：✅ `MIG-02 wave#81` 完成一段（router builder default_dec 默认决策去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.78-wave81-v1`；✅ 回流阻断负样例证据更新

1. **推进 wave#81（MIG-02 hardening，router builder 路径）**：
   - `crates/sb-core/src/router/builder.rs`：
     - `default_dec` 在缺失 `route.default/final_outbound` 时默认决策从 `direct` 调整为 `unresolved`，去除 silent direct fallback 字面量。
2. **strict gate allowlist 升级（V7 wave#81）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.78-wave81-v1`，断言扩展到 222 条（新增 W81-01/W81-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave81_v7_regression_block.txt`（在临时 root 将 `unwrap_or(\"unresolved\")` 注入回 `unwrap_or(\"direct\")` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave81_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave81_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave81_strict_gate.txt`，`V7 PASS (222 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave81_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave81_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave81_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave81_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave81_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave81_gui_static_syntax_check.txt`）

---

## 🆕 最新进展：L21 wave#80 推进落地（2026-03-06 01:19）

**状态**：✅ `MIG-02 wave#80` 完成一段（router analyze_rules 默认决策去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.77-wave80-v1`；✅ 回流阻断负样例证据更新

1. **推进 wave#80（MIG-02 hardening，router analyze_rules 路径）**：
   - `crates/sb-core/src/router/analyze.rs`：
     - `analyze_rules` 中规则缺失 `to` 时默认决策从 `direct` 调整为 `unresolved`，去除 silent direct fallback 字面量。
2. **strict gate allowlist 升级（V7 wave#80）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.77-wave80-v1`，断言扩展到 220 条（新增 W80-01/W80-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave80_v7_regression_block.txt`（在临时 root 将 `unwrap_or(\"unresolved\")` 注入回 `unwrap_or(\"direct\")` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave80_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave80_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave80_strict_gate.txt`，`V7 PASS (220 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave80_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave80_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave80_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave80_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave80_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave80_gui_static_syntax_check.txt`）

---

## 🆕 最新进展：L21 wave#79 推进落地（2026-03-06 01:11）

**状态**：✅ `MIG-02 wave#79` 完成一段（sb-subscribe parse_singbox 决策去 silent default fallback）；✅ strict gate allowlist 升级到 `l21.76-wave79-v1`；✅ 回流阻断负样例证据更新

1. **推进 wave#79（MIG-02 hardening，sb-subscribe rule mapping 路径）**：
   - `crates/sb-subscribe/src/parse_singbox.rs`：
     - `map_rule` 中缺失 outbound 时的默认决策从 `default` 调整为 `unresolved`，去除 silent default fallback 字面量。
2. **strict gate allowlist 升级（V7 wave#79）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.76-wave79-v1`，断言扩展到 218 条（新增 W79-01/W79-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave79_v7_regression_block.txt`（在临时 root 将 `unwrap_or(\"unresolved\")` 注入回 `unwrap_or(\"default\")` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave79_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave79_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave79_strict_gate.txt`，`V7 PASS (218 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave79_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave79_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave79_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave79_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave79_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave79_gui_static_syntax_check.txt`）

---

## 🆕 最新进展：L21 wave#78 推进落地（2026-03-06 01:07）

**状态**：✅ `MIG-02 wave#78` 完成一段（router analyze_fix 默认决策去 silent direct fallback）；✅ strict gate allowlist 升级到 `l21.75-wave78-v1`；✅ 回流阻断负样例证据更新

1. **推进 wave#78（MIG-02 hardening，router analyze_fix 路径）**：
   - `crates/sb-core/src/router/analyze_fix.rs`：
     - `build_portrange_merge_patch` 在缺失决策标记时默认值从 `direct` 调整为 `unresolved`，去除 silent direct fallback 字面量。
2. **strict gate allowlist 升级（V7 wave#78）**：
   - `agents-only/06-scripts/l20-migration-allowlist.txt` 升级到 `l21.75-wave78-v1`，断言扩展到 216 条（新增 W78-01/W78-02）。
   - 回流阻断证据：`reports/l21/artifacts/wave78_v7_regression_block.txt`（在临时 root 将 `\"unresolved\".into()` 注入回 `\"direct\".into()` 后，`--v7-only` 预期失败，`exit_code=1`）。
3. **门禁与编译复验**：
   - `cargo check -p app --tests`：PASS（`reports/l21/artifacts/wave78_wp1_app_tests_check.txt`）。
   - `cargo check -p sb-core`：PASS（`reports/l21/artifacts/wave78_wp1_sb_core_check.txt`）。
   - `bash agents-only/06-scripts/check-boundaries.sh --strict`：PASS（`reports/l21/artifacts/wave78_strict_gate.txt`，`V7 PASS (216 assertions)`）。
4. **L18 隔离下静态回归**（不跑运行流程）：
   - `bash -n scripts/l18/gui_real_cert.sh`：语法通过（`reports/l21/artifacts/wave78_gui_static_syntax_check.txt`）。

**最小验证**：
1. `cargo check -p app --tests`（`wave78_wp1_app_tests_check.txt`）
2. `cargo check -p sb-core`（`wave78_wp1_sb_core_check.txt`）
3. `bash agents-only/06-scripts/check-boundaries.sh --strict`（`wave78_strict_gate.txt`）
4. `BOUNDARY_PROJECT_ROOT=<tmp> ... bash agents-only/06-scripts/check-boundaries.sh --v7-only`（预期 FAIL，见 `wave78_v7_regression_block.txt`）
5. `bash -n scripts/l18/gui_real_cert.sh`（`wave78_gui_static_syntax_check.txt`）
