...
- [x] census 文档覆盖全部 endpoint 与文件，SUSPECT-DEAD 判定均有 grep 证据。
- [x] D13 执行完毕：app 内不再有自建 HTTP server/auth 栈
      （`grep -rn "http_server" app/src/admin_debug` 仅剩扩展点注册代码或为空）。
- [x] `app/src/admin_debug/mod.rs` 头部无整页 blanket allow；
      `cargo clippy -p app --features admin_debug --all-targets` 全绿。
- [x] `--features acceptance` 构建 + 现有 admin_debug 相关测试全绿。
- [x] 被保留 endpoint 的行为回归：审计/限流/auth 关键路径各 ≥1 条测试锁定。
- [x] 全局验收门禁五连全绿。
...
全部 endpoint 保留；无 SUSPECT-DEAD 删除。app 自建 HTTP server/auth/middleware 已拆除，
canonical owner 为 `sb_api::debug`；app 只留状态、endpoint 与 router extension。
`admin_auth_contract` 实网 loopback 锁定 200/401/429、request-id 与 envelope；config PUT
锁定 audit。验收补修测试 feature 门控，避免仅启用 `admin_debug` 时引用
`sb-api/admin_tests` 专用 helper。boundary W188/W189 断言已迁到新 owner。
