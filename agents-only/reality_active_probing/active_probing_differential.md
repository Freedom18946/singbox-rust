...
- **反向双核 fixture**（Go-client ↔ Rust-server）：empirical 跨核验证需完整 Go 编排（现有 fixture 仅
  Go-server↔Rust-client），登记为 NON-gating 跟进。fork patch 使 Rust server 与 Go server 对称
  （同发 ed25519 CertVerify），Go client 高概率接受（同一容忍代码），但本轮未 empirical run。
