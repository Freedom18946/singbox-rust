...
> ✅ **当前门禁状态（2026-06-06 起）**：`check-boundaries.sh`（严格）**exit 0**（V7 537 断言，0 违规）；
> 此前登记的脚本陈旧目标漂移（`bootstrap.rs`/`run_engine.rs` 迁移提示、已拆分的
> `validator/v2.rs`→`validator/v2/` 失配）已修复。预算桶定义仍有效。实时门禁状态见 `active_context.md`。
