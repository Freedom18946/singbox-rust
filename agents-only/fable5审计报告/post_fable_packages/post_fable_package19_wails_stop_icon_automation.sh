...
print("pf19-" + secrets.token_urlsafe(18))
PY
)}"
...
CONFIRM_APP_SUPPORT_OVERWRITE="${PF19_CONFIRM_APP_SUPPORT_OVERWRITE:-}"
...
require_app_support_write_confirmation() {
  ...
  if [ "$CONFIRM_APP_SUPPORT_OVERWRITE" != "YES" ]; then
    ...
    set_status "BLOCKED_CONFIRMATION" "set PF19_CONFIRM_APP_SUPPORT_OVERWRITE=YES to allow replacing $APP_SUPPORT"
    finish 0
  fi
