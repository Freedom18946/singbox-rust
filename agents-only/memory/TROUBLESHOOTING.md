...
### 规划中的 boundary 断言数与当前脚本不符

不要把旧数字强行写回脚本。先运行 `make boundaries`，确认当前 V7 registry 版本与
有效断言数；再把 MIG 专属终局不变量放入独立策略层。MIG-03 实例：规划写 537，
实际 V7 为 427；终局约束放 V8，避免污染已关闭 L20/L21 registry。
