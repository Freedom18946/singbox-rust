...
- [ ] `grep -rn "pub trait OutboundConnector" crates/ app/` 输出仅 sb-types 一处
      （测试代码中的 mock trait 除外，逐个说明）。
- [ ] `outbound/traits.rs` 删除；switchboard/pipeline/types 的自有 trait 定义为 0。
- [ ] WP02 遗留的集中式过渡适配已移除，或包内注明移除排期（不得晚于 WP06）。
- [ ] sb-proto 已按 D15 删除（类型并入 sb-types），workspace/boundary/导航文档
      同步完毕；无未升级的 D18 冲突项。
- [ ] 全局验收门禁五连全绿；`cargo test -p sb-core` 全绿。
- [ ] 双核冒烟：interop 现有 route/socks case 无新增差分（S4 归因流程）。
