...
- [ ] `mig03_wp01_trait_census.md` 覆盖 Primary evidence 全部条目，每条含
      实现者/调用方计数与可复现 grep 命令。
- [ ] `mig03_adr01_canonical_connector.md` 完成，a–h 八问全部落到精确签名，
      与 D1–D8 的逐条对照表在 ADR 内，含 ≥2 个被否决备选的记录。
- [ ] 迁移映射表无 TBD 项。
- [ ] 无未升级的 D 条目冲突项（有升级则记录用户答复后才勾）。
- [ ] `git status` 确认本包只新增了 `agents-only/mig03/` 下的文档。
...
（执行时填写：本包过程中发现但不属于本包范围的问题，逐条列出并注明建议去向。）
