...
- 追加静态审计结论：
  ...
  - `app/src/admin_debug/security_metrics.rs` / `app/src/logging.rs` 的默认全局 owner 已收敛为 `Weak` 注册表；其中 subs/prefetch/breaker/security_async 主链、真实 admin server 的 `/subs/` 入口以及 `main` logging 启动/退出路径都已优先改走显式 owner
