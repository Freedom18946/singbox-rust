...
- 本轮新增完成：
  ...
  - `crates/sb-core/src/router/mod.rs`、`crates/sb-core/src/router/engine.rs`
    - `decide_udp_with_rules()` 的 GeoIP helper 分支现优先走 `RouterHandle` owner-first lookup
    - 新增 `decide_udp_with_rules_and_handle(...)` 供显式 router owner 直接复用；compat `crate::geoip` lookup 仅保留为未命中时回退
  - `crates/sb-core/src/geoip/mod.rs`
    - 旧的强全局安装路径继续保留兼容
    - 新增默认弱 owner 注册表；内部 lookup 先走显式 owner，再 fallback 到旧全局安装路径
    - 新增 `lookup_country_code(...)` 作为单次 owner-first country lookup helper
  - `crates/sb-core/src/router/mod.rs`、`crates/sb-core/src/geoip/mod.rs`
    - UDP GeoIP compat fallback 现先复用单次 `lookup_country_code(...)` 结果再匹配规则
    - 不再按每条 GeoIP 规则重复打 compat lookup
  ...
  - `app/src/bin/metrics-serve.rs`、`crates/sb-metrics/examples/serve.rs`
    - standalone metrics serve / example 路径现改为显式持有 `MetricsRegistryOwner`
    - 不再把 `MetricsRegistryHandle::global()` 当作这些入口的首选 owner 来源
  - `app/src/runtime_deps.rs`
    - `metrics_registry()` 现直接返回 `MetricsRegistryOwner::handle()`
    - runtime exporter 路径不再经由 `shared_registry()` compat handle
...
- 追加静态审计结论：
  ...
  - `app/src/admin_debug/security_metrics.rs` / `app/src/logging.rs` 的默认全局 owner 已收敛为 `Weak` 注册表；其中 subs/prefetch/breaker/security_async 主链、CLI prefetch 路径、真实 admin server 的 `/subs/` 入口、`Prefetcher` runtime 路径以及 `main` logging 启动/退出路径都已优先改走显式 owner；legacy `enqueue_prefetch(...)` / async DNS helper 也已改成入口处升级 owner 后继续沿显式 `Arc` 透传
  - `crates/sb-core/src/geoip/mod.rs` — **hard global 已删除**（2026-03-25）：`GEOIP_SERVICE` OnceLock + `init()` + `service()` 已移除。`lookup_country_code()` 只走 `DEFAULT_GEOIP_SERVICE` weak-owner lookup，不再回落 hard global。Router-local provider（`RouterHandle::enhanced_geoip_lookup`）不受影响。
  - `app/src/admin_debug/prefetch.rs` — **hard global + async lifecycle 均已收口**（2026-03-25）：`GLOBAL` / `Prefetcher::global()` / `global_take()` 已删除。Worker 改为单 dispatcher（直接持有 `Receiver`）+ `CancellationToken` + `JoinSet`。不再有 `Arc<Mutex<Receiver>>` 锁跨 await，不再有裸 spawn。`shutdown()` 是真实 async shutdown。仅剩 `DEFAULT_PREFETCHER` weak-owner compat。
  - `crates/sb-metrics/src/lib.rs` 的 shared registry owner 已收敛为 `AppRuntimeDeps`、`metrics-serve` binary 和 `sb-metrics` example 的显式持有；`AppRuntimeDeps::metrics_registry()` 也已直接返回 owner handle；`shared_registry()` 现保留“弱默认 owner 优先、强全局 fallback，并合并 owner 安装前旧全局指标”的兼容入口；`export_prometheus()` / `maybe_spawn_http_exporter_from_env()` / `sb-core` legacy `run_exporter()` 也已退到显式 handle helper 外侧的 compat 壳
