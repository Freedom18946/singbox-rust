#!/usr/bin/env zsh
set -euo pipefail
echo "[INFO] e2e: proxy health metrics with failing upstream"
...
echo "[INFO] Health check enabled"
...
# 6) Test HTTP connection. Success is not required; metrics are the primary signal.
echo "[INFO] Testing HTTP connection via failing proxy"
set +e
...
if [ $CURL_EXIT -eq 0 ]; then
    echo "[OK] HTTP connection succeeded"
else
...
if [[ -n "$route_total" ]]; then
    ...
    echo "[OK] Direct routes recorded"
else
...
echo "[INFO] Summary: Proxy health check system tested"
echo "[DONE]"
