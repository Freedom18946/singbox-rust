...
generate_version_metadata() {
    ...
    if [[ -x "${ROOT}/target/${PROFILE}/sb-version" ]]; then
        sb_version_output=$("${ROOT}/target/${PROFILE}/sb-version" 2>/dev/null || echo "{}")
    elif [[ -x "${ROOT}/target/debug/sb-version" ]]; then
        sb_version_output=$("${ROOT}/target/debug/sb-version" 2>/dev/null || echo "{}")
    else
        sb_version_output="{}"
    fi
...
main() {
    ...
    if [[ ! -x "${ROOT}/target/${PROFILE}/sb-version" && ! -x "${ROOT}/target/debug/sb-version" ]]; then
        log "Building sb-version binary..."
        cd "${ROOT}" && cargo build --bin sb-version
    fi
